---
name: verify
description: SlipKit 검증 게이트를 실행한다 (TSDoc 형식 lint + 전 패키지 typecheck·build·test). 커밋 전 확인 또는 사용자가 검증을 요청할 때 사용.
allowed-tools:
  - "Bash(pnpm lint)"
...
1. `pnpm lint` — TSDoc 주석 형식 검사 (tsdoc/syntax, ADR-030)
2. `pnpm -r typecheck`
3. `pnpm -r build`
4. `pnpm -r test`

- 네 단계 모두 통과해야 커밋 가능 상태다. 실패하면 원인을 고친다 —
