...
- **REQUIREMENTS.md**: ADR 추가·변경 시 반드시 함께 갱신해 일치하도록 유지한다.
- **OPEN-QUESTIONS.md**: 새 쟁점은 다음 Q 번호로 추가하고 사용자 확인 전에 임의로 결정하지 않는다.
  해결된 쟁점은 삭제하지 않고 취소선 + 해결 ADR 링크.
- **SPEC.md**: `.slip` 포맷의 공개 규범 — 구현(`@slipkit/core`)과 어긋나면 SPEC이 우선.
  포맷 변경 시 Zod 스키마·마이그레이션·JSON Schema와 반드시 동시 갱신 (core 규칙 참조).
