...
- **PR 제목·커밋 제목은 업무 문서처럼 담백하게 쓴다** (사용자 지정 2026-08-21).
...
- **PR을 만들거나 본문을 다시 쓰기 전에는 사용자에게 모델 상태 확인을 요청하고 기다린다**
  (사용자 지정 2026-08-21). 본문 작성에 쓸 모델은 사용자가 정하며, 전환은 사용자만 할 수 있다.
  구현·커밋·푸시까지는 그대로 진행하고, 본문 작성 직전에 멈춰 요청한다.
  다만 **이미 있는 본문에 항목을 덧붙이거나 수치·표를 고치는 정도의 보완은 요청 없이 직접** 한다 —
  대신 본문을 갈아엎지 않는다 (사용자 지정 2026-08-21).
- PR 본문·커밋 메시지에 "사용자 요청으로" 같은 요청 출처 표기를 쓰지 않는다.
  요청으로 시작된 작업도 기본 작업과 똑같이 변경 내용만 서술한다 (사용자 지정 2026-08-19).
- "화면 점검 피드백 반영" 같은 **피드백·지적 출처 표기도 쓰지 않는다** — 여러 사람이
  참여한 듯한 뉘앙스를 배제하고, 이전 작업에 대한 추가 수정·개선으로만 서술한다
  (사용자 지정 2026-08-20). 문서(ROADMAP 등)도 동일하게 쓴다.
