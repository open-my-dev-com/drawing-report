 ...
 * 2) 검증 게이트: git commit 전에 `pnpm -r typecheck && build && test`를
...
if (hasGitCommit) {
  ...
  // 검증 게이트 — 실패 시 커밋 차단
  try {
    execSync('pnpm -r typecheck && pnpm -r build && pnpm -r test', {
      cwd,
      encoding: 'utf8',
    ...
    block(
      `[bash-guard] 검증 게이트 실패 — 커밋이 차단되었습니다 (CLAUDE.md).\n` +
        `pnpm -r typecheck && pnpm -r build && pnpm -r test 가 전부 통과해야 커밋할 수 있습니다.\n` +
