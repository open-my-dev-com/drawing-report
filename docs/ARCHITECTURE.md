...
flowchart TB
    ...
    CustomFonts["사용자 지정 폰트<br/>TTF·OTF"]
...
애플리케이션 단위 설정이 필요할 때는 `createSlipKit()`으로 인스턴스를 생성할 수 있습니다. 폰트 제공 함수, 로케일, 암호화 키와 이전 키 등을 인스턴스 설정으로 전달합니다. 컴포넌트와 저장 수단은 이 인스턴스를 받아 같은 설정을 재사용합니다(ADR-064).
...
파일 접근은 해석된 작업 디렉터리 안으로 제한합니다. 읽기 응답은 요약을 기본으로 하고 Base64 이미지 데이터를 제외합니다. 기존 파일은 요소 ID·파라미터 키·페이지 번호로 지정한 연산만 적용하고, 최종 결과가 전체 검증을 통과할 때만 저장합니다.
...
| 확장 지점 | 책임 |
| --- | --- |
| 폰트 제공 함수 | PDF 생성에 사용할 폰트 데이터를 반환합니다. |
