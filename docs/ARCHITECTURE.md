...
ADR-054(파일 암호화) · ADR-022(SPEC·JSON Schema 공개).
...
2. 기본 패턴은 **브라우저가 엔진**이다 — 수식·PDF·발행(확정)은 사용자 브라우저 안의 core가 수행하고,
...
flowchart TB
    subgraph BROWSER["브라우저 — 호스트 웹앱"]
        ...
        subgraph KIT["SlipKit (npm 설치)"]
            ...
            CORE["core (순수 TS)<br/>파싱·검증 · 수식 평가<br/>PDF 렌더 · 파일 암호화"]
...
sequenceDiagram
    ...
    J-->>B: values (JSON)
    Note over B: ③ core 실행: 전표 조립 → 수식 계산<br/>→ 발행(확정) → PDF 생성·인쇄
    B->>J: ④ 발행된 전표 .slip 전송
...
Node 20+에서 그대로 돌기 때문에(결합 테스트가 Node에서 PDF 생성까지 수행해 확인됨)
