...
| 하고 싶은 일 | 문서 | 대상 |
|---|---|---|
| 저장소에서 SlipKit을 처음 실행하고 디자이너 연결하기 | [시작하기](getting-started.ko.md) | 처음 사용하는 개발자 |
| 디자이너·작성 폼·뷰어를 연결하고 결과 저장하기 | [애플리케이션 통합 가이드](integration.ko.md) | 프런트엔드·애플리케이션 개발자 |
| 디자이너 화면에서 양식 만들기 | [양식 디자이너 사용 가이드](designer.ko.md) | 양식 제작자 |
| `.slip` 파일 처리, 전표 조립과 PDF 생성하기 | [Core 사용 가이드](core.ko.md) | 백엔드·Core 개발자 |
| Node.js 서버에서 전표를 발행하고 PDF를 생성·보관하기 | [서버 통합 가이드](server-integration.ko.md) | 백엔드 개발자 |
| MCP 서버를 설정하고 AI로 `.slip` 양식을 만들고 고친 뒤 PDF로 확인하기 | [MCP 사용 가이드](mcp.ko.md) | MCP 사용자·AI 연동 개발자 |
| 언어, 폰트, 용지, 프리셋과 저장소 설정하기 | [환경 설정 가이드](configuration.ko.md) | 애플리케이션 개발자 |
| 양식에서 계산식 작성하기 | [수식 함수 참조](formula.ko.md) | 양식 제작자·개발자 |
| 공개 함수, 타입, 컴포넌트와 이벤트 확인하기 | [API 참조](api-reference.ko.md) | 연동 개발자 |

> [!TIP]
> 처음 사용한다면 [시작하기](getting-started.ko.md)를 먼저 진행한 뒤 [애플리케이션 통합 가이드](integration.ko.md)로 넘어가세요.
>
> 양식 제작만 담당한다면 [양식 디자이너 사용 가이드](designer.ko.md)를, UI 없이 `.slip` 파일과 PDF를 처리한다면 [Core 사용 가이드](core.ko.md)를 바로 확인해도 됩니다.
...
- 발행된 전표는 `issued: true`이며 작성 폼에서 더 이상 수정할 수 없습니다.
...
편집 결과는 컴포넌트가 자동으로 영구 저장하지 않습니다. 디자이너와 작성 폼이 전달하는 이벤트를 애플리케이션에서 받아 저장해야 합니다.
...
| 전표 | 양식에 실제 값을 입력한 파일 |
| 발행 | 작성한 전표의 값을 확정하여 더 이상 작성 폼에서 수정할 수 없게 만드는 작업 |
...
패키지 구성, 로컬 실행 방법, 개발 명령어, 기술 문서와 라이선스는 [프로젝트 README](../../README.ko.md)를 확인하세요.
