...
| 항목 | 용도 |
|---|---|
| 키 | 파일, 수식과 외부 시스템 연계에 사용하는 이름 |
| 표시 이름 | 작성 폼과 디자이너 화면에 표시하는 이름 |
| 파라미터 값 종류 | 입력 방식과 수식에서의 값 종류 |

지원하는 파라미터 값 종류는 다음과 같습니다.
...
> [!TIP]
> 키는 화면에 표시하는 문장이 아니라 데이터 연계에 사용하는 식별자입니다.
> `거래 일자`보다 `tradeDate`처럼 공백 없이 의미를 알 수 있는 이름을 권장합니다.

키를 변경하면 해당 파라미터를 참조하는 요소와 샘플 값도 함께 변경됩니다. 이미 사용 중인 키와 같거나 비어 있는 이름은 사용할 수 없습니다.
...
- 요소를 선택한 상태에서 방향키를 누르면 0.5mm, <kbd>Shift</kbd>+방향키는 5mm 이동합니다. 키를 누르고 있으면 계속 이동하며, 키를 뗄 때까지의 이동은 한 번에 되돌립니다.
- 요소는 용지의 오른쪽·아래쪽 경계를 넘어 배치할 수 있습니다. 용지 밖 부분은 PDF에 출력되지 않으며, 그런 요소를 선택하면 설정 패널에 안내가 표시됩니다. 왼쪽·위쪽 경계는 넘을 수 없고, 여러 요소를 함께 옮기면 가장 바깥 요소가 경계에서 멈추면서 요소 사이 간격은 유지됩니다.
...
캔버스나 사이드바의 요소 목록에서 <kbd>Shift</kbd> 또는 <kbd>Ctrl</kbd>(macOS는 <kbd>⌘</kbd>)을 누른 채 요소를 클릭하면 선택에 추가하거나 제외합니다. 그룹은 항상 하나의 단위로 선택됩니다.

선택된 요소 중 하나를 드래그하면 선택 전체가 함께 이동하고, 그룹으로 묶을 수도 있습니다. 그룹으로 묶으면 그룹 안의 요소 하나를 선택해도 전체가 함께 선택됩니다.

여러 요소를 선택하면 설정 패널에 정렬 버튼(왼쪽·가로 가운데·오른쪽·위·세로 가운데·아래)과 동일 간격 버튼(가로·세로)이 표시됩니다. 정렬은 선택 전체의 바깥 경계를 기준으로 하고, 동일 간격은 요소나 그룹이 3개 이상일 때 첫 번째와 마지막을 고정한 채 사이 간격을 같게 맞춥니다. 명령 한 번은 되돌리기 한 단계입니다.
...
감쌉니다. 기본값에서는 그리드 테두리를 표시하지 않습니다.
...
| 셀 타입 | 용도 |
|---|---|
| 직접 입력 | 열 제목이나 모든 전표에 같은 내용으로 표시할 글 |
| 파라미터 | 전표에서 입력받은 값 |
| 수식 | 다른 값으로 계산한 결과 |
...
5. <kbd>행 구간</kbd>에서 항목마다 반복할 행을 <kbd>항목 구간</kbd>으로 지정합니다.
6. 항목 구간의 각 셀에 목록의 하위 필드를 연결합니다.
7. 페이지 방식을 선택합니다.
8. 그리드 설정 패널 제목 줄의 <kbd>출력 결과 보기</kbd>로 실제 반복 결과를 확인합니다. 캔버스 위치는 그대로 유지되고, 편집 동작을 하면 자동으로 행 구조 편집으로 돌아옵니다. <kbd>Esc</kbd>로도 돌아올 수 있습니다.
...
실제 데이터가 있지만 첫 항목이나 고정 페이지 묶음을 첫 페이지의 남은 공간에 하나도 놓을 수 없으면
빈 그리드 조각을 남기지 않고 다음 페이지의 흐름 영역 위쪽에서 시작합니다. 이어지는 페이지의 전체
흐름 영역에도 들어가지 않는 항목이나 고정 묶음은 배치 오류입니다.

`after` 배치에서는 그리드의 시작 위치가 줄어들기 때문에 실제 데이터가 없어도 같은 이동이 일어날 수
있습니다. 그 밖에는 고정 페이지나 최소 표시 항목 수가 만드는 빈 항목을 그리드의 원래 위치에 유지합니다.
...
> [!IMPORTANT]
> 항목 구간의 셀에는 목록 전체인 `items`가 아니라 `items` 안의 하위 필드를 연결합니다.
...
수식 입력란과 계산 결과는 편집 창 상단에 전체 폭으로 놓입니다. 그 아래의 <kbd>함수</kbd>·<kbd>값</kbd> 탭에서 참조를 삽입할 수 있으며, 함수를 고르면 목록 옆에 상세 설명이 나타나고 삽입하면 커서가 원래 자리로 돌아옵니다.
...
항목 구간의 셀은 현재 반복 항목의 하위 필드를 조건식에서 직접 참조할 수 있습니다. 예를 들어 금액 셀의 조건식에 `amount < 0`을 입력하면 각 행의 `amount` 값으로 규칙을 평가합니다.
...
폼과 JSON 모드를 전환하거나 변경 없이 적용해도 기존 샘플 데이터의 키와 값은 유지됩니다. 여기에는
파라미터로 선언하지 않은 키, `__proto__`·`constructor`·`toString` 같은 키와 형식이 잘못된 기존
목록 값도 포함됩니다. 선언됐지만 샘플 값이 없는 파라미터에만 편집용 기본값을 추가합니다.
...
- <kbd>파일 열기</kbd>
- 새로고침 후 작업 복원
- <kbd>양식 만들기</kbd>와 <kbd>전표 작성</kbd> 화면 전환
- 헤더 아래의 저장 안내와 <kbd>저장 데이터 삭제</kbd> 버튼

실제 애플리케이션에서 이러한 기능을 구현하는 방법은 [애플리케이션 통합 가이드](integration.ko.md)를 참고하세요.

### 브라우저에 저장되는 데이터

데모는 작업 중인 양식과 전표를 이 브라우저에 저장해 다음에 열 때 그대로 복원합니다. 헤더 아래의 안내가 이 사실을 알리며 작업하는 동안 계속 보입니다.

같은 안내 영역의 <kbd>저장 데이터 삭제</kbd>는 데모가 자동 저장한 양식, 작성 중 전표, 발행된 전표와 마지막 화면 정보만 삭제합니다. <kbd>내 양식</kbd>에 직접 저장한 양식은 유지됩니다. 삭제 전에는 확인 대화상자가 표시되며, 삭제한 뒤에는 동봉된 초기 양식과 빈 전표로 돌아갑니다. 이미 내려받은 파일과 브라우저의 다른 저장 항목은 삭제하지 않습니다. 다른 양식을 열 때 이전 양식의 작성 중 전표가 함께 복원되지 않도록 자동 저장된 전표도 삭제합니다.

> [!WARNING]
> 키를 설정하지 않으면 데모는 소스 코드에 공개된 샘플 키로 데이터를 암호화합니다. 이 키는 개인정보를 보호하지 못합니다. 데모에 실제 개인정보나 기밀 정보를 입력하지 말고, 공용 기기에서는 사용을 마친 뒤 저장 데이터를 삭제하세요.

실제 애플리케이션은 데이터를 어디에 둘지와 사용자가 어떻게 지울지를 스스로 정합니다. [애플리케이션 통합 가이드](integration.ko.md)를 참고하세요.
...
직접 입력한 글과 수식 결과는 사용자가 작성 폼에서 입력하는 값이 아니므로 입력 항목이 되지 않습니다.
