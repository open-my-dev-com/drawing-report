...
같은 안내의 <kbd>저장 데이터 삭제</kbd>는 데모가 저장한 것 — 양식, 작성 중 전표, 발행된 전표와 기억된 화면 — 을 지웁니다. 되돌릴 수 없으므로 먼저 확인을 받고, 지운 뒤에는 동봉된 처음 양식으로 돌아갑니다. 이미 내려받은 파일은 그대로 남고 브라우저의 다른 저장 항목은 건드리지 않습니다.
