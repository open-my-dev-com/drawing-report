...
같은 안내의 <kbd>저장 데이터 삭제</kbd>는 데모가 자동 저장한 양식, 작성 중 전표, 발행된 전표와 기억된 화면만 지웁니다. <kbd>내 양식</kbd>에 명시적으로 저장한 양식은 남습니다. 되돌릴 수 없으므로 먼저 확인을 받고, 지운 뒤에는 동봉된 처음 양식과 빈 전표로 돌아갑니다. 이미 내려받은 파일은 그대로 남고 브라우저의 다른 저장 항목은 건드리지 않습니다. 다른 양식을 열 때도 이전 양식의 자동 저장 전표가 함께 복원되지 않도록 작성 중 전표를 지웁니다.

> [!WARNING]
> 키를 설정하지 않으면 데모는 자기 소스에 공개된 샘플 키로 암호화합니다. 이 키는 아무것도 지켜 주지 않습니다 — 데모에 실제 개인정보나 대외비 내용을 입력하지 말고, 공용 기기에서는 사용을 마친 뒤 저장 데이터를 삭제하세요.
