...
그룹 소계를 사용하려면 <kbd>고급 설정</kbd>에서 그룹 기준 필드를 먼저 선택합니다. 빠른 구성으로 추가한 행도 행 구간 목록에서 범위, 역할과 표시 페이지를 다시 바꿀 수 있습니다.

캔버스의 행 번호를 선택하면 해당 행의 역할을 바로 바꿀 수 있습니다. 여러 행을 한 구간으로 묶으려면 <kbd>Shift</kbd>를 누른 채 다른 행 번호를 선택합니다. 같은 작업은 오른쪽 패널의 행 구간 목록에서도 할 수 있습니다.
