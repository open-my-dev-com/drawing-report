...
사이드바의 요소 목록에서 그리드를 펼치면 파라미터나 수식이 연결된 셀로 바로 이동할 수 있습니다.

### 반복 목록 만들기

품목 표처럼 데이터 개수만큼 행이 반복되는 표는 <kbd>구간 반복</kbd>을 사용합니다.

1. 목록 파라미터와 하위 필드를 먼저 정의합니다.
2. 그리드에 헤더 행과 반복에 사용할 행을 만듭니다.
3. 그리드 설정에서 <kbd>반복 사용</kbd>을 켭니다.
4. 시작 행과 종료 행을 지정합니다.
5. 반복에 사용할 목록 파라미터를 선택합니다.
6. 반복 구간의 각 셀에 목록의 하위 필드를 연결합니다.
7. 페이지당 표시할 행 수를 정합니다.
8. 필요하면 표시 행 수 상한과 페이지 전환 시 헤더 반복을 설정합니다.
...
> [!IMPORTANT]
> 반복 구간의 셀에는 목록 전체인 `items`가 아니라 `items` 안의 하위 필드를 연결합니다.

> [!TIP]
> 반복 행이 여러 줄로 구성된 카드 형태라면 시작 행과 종료 행을 다르게 설정할 수 있습니다. 지정한 행 구간 전체가 목록 항목마다 반복됩니다.
...
반복 구간 안의 셀은 현재 반복 항목의 하위 필드를 조건식에서 직접 참조할 수 있습니다. 예를 들어 금액 셀의 조건식에 `amount < 0`을 입력하면 각 행의 `amount` 값으로 규칙을 평가합니다.
