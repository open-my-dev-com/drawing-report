...
> 전체 목록은 `$(items).$(amount)`로 참조합니다. 기존 `items.amount` 수식도 계속 동작합니다.
...
키 안의 `)`는 `\)`, `\`는 `\\`로 적습니다. 기존 수식의 `totalAmount`와 `items.amount` 같은
일반 참조는 계속 사용할 수 있습니다. 새 수식은 `$(...)`를 사용하며, 한 수식 안에서 일반 참조와
명시형 데이터 참조를 섞을 수 없습니다. 반복 그리드 예약 범위는 괄호 없이
`@item.$(amount)`처럼 씁니다. `$(@item)`은 예약 참조가 아니라 이름이 `@item`인 업무 키입니다.
...
토큰은 긴 것부터 먼저 해석합니다. 영문을 그대로 표시하려면 `[...]`로 감쌉니다. 한글, 공백과
문장 부호는 대괄호 밖에서도 그대로 표시됩니다. 대괄호 안에서는 `\]`를 `]`로, `\\`를 `\`로
해석합니다. 알 수 없는 영문 토큰, 잘못된 이스케이프와 닫히지 않은 대괄호는 오류입니다.
