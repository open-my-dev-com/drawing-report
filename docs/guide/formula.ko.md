...
[English](formula.md) · [日本語](formula.ja.md)
...
| 항목 구간 밖의 그리드 셀 | 전표의 전체 값 | 표 하단 합계 |
| 항목 구간 안의 그리드 셀 | 전표의 전체 값과 현재 반복 항목 | 행별 금액 계산 |
| 텍스트·필드·항목 구간 밖 셀의 조건부 서식 | 전표의 전체 값 | 음수 금액이나 특정 상태 강조 |
| 항목 구간 안 셀의 조건부 서식 | 전표의 전체 값과 현재 반복 항목 | 조건에 맞는 행 강조 |
...
표시 수식은 [양식 디자이너 사용 가이드의 수식 작성](designer.ko.md#7-수식-작성하기), 조건식은 [조건부 서식 설정](designer.ko.md#8-조건부-서식-설정하기)을 확인하세요.
...
> [!IMPORTANT]
> 항목 구간 안에서는 현재 항목의 필드가 같은 이름의 전표 최상위 값보다 우선합니다.
...
| 하위 필드 | 각 키를 `$()`로 감싸고 마침표로 연결 | `$(customer).$(name)` |
...
지정한 자릿수로 반올림합니다. 자릿수를 생략하면 정수로 반올림합니다. 정확히 절반인 값은 0에서 멀어지는 방향으로 사사오입하므로 `ROUND(-2.5)`는 `-3`입니다.

자릿수는 -20 이상 20 이하의 정수여야 하며 음수를 지정하면 십·백·천 단위로 반올림할 수 있습니다.
...
표시 결과는 수식 계산 문맥의 `locale` 설정을 따릅니다. `locale`을 지정하지 않으면 `en-US`를 사용합니다.
...
로케일 설정 방법은 [환경 설정 가이드](configuration.ko.md#ui-언어-설정)를 확인하세요.
...
직접 수식을 계산할 때는 계산 문맥에 `now`를 전달하여 결과를 재현할 수 있습니다.

자세한 사용 방법은 [Core 사용 가이드](core.ko.md#수식-평가하기)를 확인하세요.
...
| 문제 | 확인할 내용 |
|---|---|
| 숫자 계산에서 값 종류 오류가 발생함 | 파라미터의 값 종류가 숫자인지 확인하고, 문자열이면 `TO_NUMBER` 사용 |
...
- [ ] 파라미터와 목록 필드 이름을 정확히 참조했습니다.
- [ ] 숫자 계산에 숫자 타입 값을 사용했습니다.
- [ ] 문자열 숫자가 있다면 `TO_NUMBER`로 변환했습니다.
- [ ] 목록 범위를 집계 함수와 함께 사용했습니다.
- [ ] 빈 값과 빈 목록일 때의 결과를 확인했습니다.
- [ ] 세액의 반올림 또는 절사 방식을 명시했습니다.
- [ ] 날짜가 ISO 형식이며 UTC 기준임을 확인했습니다.
- [ ] 디자이너에서 샘플 데이터로 결과를 확인했습니다.
- [ ] 바코드 수식 결과가 바코드 형식 규칙과 일치합니다.
- [ ] PDF 미리보기에서 최종 표시 결과를 확인했습니다.
