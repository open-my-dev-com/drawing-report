...
읽기 응답의 base64 이미지 데이터는 크기 표시로 대체됩니다 — `data:<종류>/<하위종류>;base64,` 머리말을 갖춘 내장 값만 바꾸고, `data:text/plain,hello`처럼 payload를 그대로 담은 값과 `data:`로 시작하기만 하는 일반 문자열을 포함해 그 밖의 문자열은 그대로 돌려줍니다. `.slip` 파일의 이미지 형식은 base64 데이터 URL을 사용하며, MCP에서 이미지를 넣을 때는 `slip_edit`의 `set_image`에 작업 디렉터리 안의 파일 경로를 전달합니다. 서버가 파일을 읽어 base64 에셋으로 변환합니다.
