...
설정 파일은 원하는 위치에 둘 수 있습니다. `rootDir`과 폰트 파일 경로는 설정 파일이 있는 디렉터리를 기준으로 해석합니다. `~`는 홈 디렉터리로 확장되지 않으므로 절대 경로나 올바른 상대 경로를 사용하세요. 심볼릭 링크는 저장 작업에서 제외하지만, 이미 만들어진 하드 링크는 일반 파일과 구분할 수 없습니다. 민감한 파일을 가리키는 하드 링크를 작업 디렉터리에 두지 마세요.
...
| 설정 | 저장하는 내용 |
|---|---|
| `slipkit-mcp.json` | 작업 디렉터리, 로케일, 사용자 지정 폰트, 암호화 키를 읽을 환경변수 이름 |
...
| `--help`, `-h` | CLI 사용법을 stdout에 출력하고 설정을 읽거나 서버를 시작하지 않은 채 종료합니다. |
| `--version`, `-v` | npm 패키지 버전을 stdout에 출력하고 서버를 시작하지 않은 채 종료합니다. |
...
알 수 없는 옵션, 옵션 값 누락과 두 개 이상의 작업 디렉터리 위치 인자는 사용법 오류입니다. CLI는 원인과 `--help` 안내를 stderr에 출력하고 종료 코드 2를 반환합니다. 설정 및 서버 시작 오류는 종료 코드 1을 반환합니다. 서버 실행 중에는 stdout을 MCP 프로토콜 메시지 전용으로 사용하고 시작 정보는 stderr에 출력합니다.
...
`httpPort`를 설정하면 `127.0.0.1`에 바인딩된 읽기 전용 서버로 렌더링한 PDF를 제공합니다. MCP 프로세스마다 별도의 64자 토큰을 만들며 반환 URL은 `http://127.0.0.1:<포트>/<토큰>/<파일>.pdf` 형식입니다. 프로세스끼리 토큰이나 기존 링크 서버를 공유하지 않습니다. 같은 작업 디렉터리의 다른 `slipkit-mcp` 링크 서버가 지정한 포트를 사용 중이면 사용 가능한 다른 포트에서 새 서버를 시작합니다. 다른 프로그램이나 다른 작업 디렉터리의 `slipkit-mcp`가 포트를 사용 중이면 오류를 알리고 시작을 중단합니다. 작업 디렉터리 안의 PDF만 제공합니다.
...
읽기 응답에 포함된 Base64 이미지 데이터는 크기 표시로 대체됩니다. `data:<종류>/<하위종류>;base64,` 머리말이 있는 내장 값만 바꾸며, `data:text/plain,hello`처럼 데이터를 그대로 담은 값과 `data:`로 시작하는 일반 문자열은 원문을 유지합니다. `.slip` 파일의 이미지는 Base64 데이터 URL 형식입니다. MCP로 이미지를 추가할 때는 `slip_edit`의 `set_image`에 작업 디렉터리 안의 파일 경로를 전달합니다. 서버는 파일을 읽어 Base64 에셋으로 변환합니다.

이미지 데이터 검사는 이미지로 선언된 값에만 적용합니다. 이미지 요소가 참조하는 파라미터, `valueType: 'image'`인 파라미터와 같은 형식의 목록 하위 필드가 대상입니다. `data:`로 시작하는 정의되지 않은 키를 포함한 일반 데이터 문자열은 그대로 보존합니다.
...
| `add_parameter`, `set_parameter`, `remove_parameter` | 키로 지정한 파라미터 |
...
기존 파일의 작은 변경에 `slip_save` 또는 `slip_read`의 `full`을 사용하지 마세요. 요약으로 대상을 찾고 `slip_edit`에서 식별자로 수정 대상을 지정하면 관련 없는 요소가 빠지거나 바뀌는 실수를 줄일 수 있습니다.
