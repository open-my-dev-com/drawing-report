...
発行後に同じテンプレートで新しい伝票を始めるには、`reset()` メソッドを呼び出します。Web Component はメソッドを直接公開し、React ラッパーの ref は内部の `<slip-form>` 要素を受け取ります。Vue ラッパーは Vue の標準動作に従い、コンポーネント ref の `$el` で内部の `<slip-form>` 要素に届くため `formRef.value.$el.reset()` で呼び出せます。コンポーネントの `key` を変更して再マウントする方法も使えます。同じ `src` 文字列を再指定するだけでは、発行済みフォームのロックは解除されません。

`.slip` バリデーターは `values`、`sampleValues`、リスト行、未定義の業務データキーを読み込み・編集・保存の過程で保持します。これには `__proto__`、`constructor`、`toString` という名前の own プロパティも含まれます。構造オブジェクトでは、これらの名前を含む未定義プロパティを拒否します。作成フォームは不正な業務値を暗黙に修復せず保持し、ユーザーが明示的に消去または修正するまで発行を止めます。業務ルールの検証はホストアプリケーションの責任です。
