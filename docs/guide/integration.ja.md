...
発行後に同じテンプレートで新しい伝票を始めるには、`reset()` メソッドを呼び出します。Web Component はメソッドを直接公開し、React ラッパーの ref は内部の `<slip-form>` 要素を受け取ります。Vue ラッパーは要素を公開しないため、コンポーネントの `key` を変更して再マウントします。同じ `src` 文字列を再指定するだけでは、発行済みフォームのロックは解除されません。
