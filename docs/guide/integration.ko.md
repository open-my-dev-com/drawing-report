...
발행 뒤 같은 양식으로 새 전표를 시작하려면 `reset()` 메서드를 호출합니다. Web Component는 메서드를 직접 제공하고, React 래퍼의 ref는 내부 `<slip-form>` 요소를 받습니다. Vue 래퍼는 요소를 노출하지 않으므로 컴포넌트의 `key`를 바꿔 다시 마운트합니다. 같은 `src` 문자열을 다시 지정하는 것만으로는 발행된 작성폼의 잠금이 풀리지 않습니다.
