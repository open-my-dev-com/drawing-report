...
サイドバーの要素一覧でグリッドを展開すると、パラメータや数式が接続されたセルにすぐ移動できます。

### 繰り返しリストを作る

品目表のようにデータの数だけ行が繰り返される表は、<kbd>繰り返し範囲</kbd> を使います。

1. リストパラメータと項目フィールドを先に定義します。
2. グリッドにヘッダー行と、繰り返しに使う行を作ります。
3. グリッドの設定で <kbd>繰り返しを使用</kbd> をオンにします。
4. 開始行と終了行を指定します。
5. 繰り返しに使うリストパラメータを選びます。
6. 繰り返し範囲の各セルに、リストの項目フィールドを接続します。
7. 1 ページに表示する行数を決めます。
8. 必要なら、表示行数の上限と、ページ切り替え時のヘッダー繰り返しを設定します。
...
> [!IMPORTANT]
> 繰り返し範囲のセルには、リスト全体の `items` ではなく、`items` 内の項目フィールドを接続します。

> [!TIP]
> 繰り返し行が複数行で構成されるカード形式なら、開始行と終了行を別々に設定できます。指定した行範囲全体がリスト項目ごとに繰り返されます。
...
繰り返し範囲内のセルでは、現在の項目のフィールドを条件式から直接参照できます。たとえば金額セルに `amount < 0` を指定すると、各行の `amount` の値でルールを評価します。
