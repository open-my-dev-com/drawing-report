...
テンプレートデザイナーはデスクトップ専用です。ブラウザーの表示領域は 1440×810 以上、`<slip-designer>` 要素には 1280×640 以上の領域が必要です。ツールバー、サイドバー、キャンバスは同じ 3 列レイアウトを維持し、モバイルまたはタブレット向けのレイアウトには切り替わりません。

ホストアプリケーションが 1280×640 の領域を確保できない場合は、デザイナーの外側のコンテナーに `overflow: auto` を指定してください。対応サイズより小さい領域でも、デザイナーが警告を表示したり編集を止めたりすることはありません。
...
| 項目 | 用途 |
|---|---|
| キー | ファイル、数式、外部システム連携に使う名前 |
| 表示名 | 作成フォームとデザイナー画面に表示する名前 |
...
> [!TIP]
> キーは画面に表示する文章ではなく、データ連携に使う識別子です。
> `取引 日付` より `tradeDate` のように、空白なしで意味の分かる名前を推奨します。

キーを変更すると、そのパラメータを参照する要素とサンプル値も一緒に変更されます。すでに使われているキーと重複する名前や空の名前は使えません。
...
- 要素を選択した状態で矢印キーを押すと 0.5mm、<kbd>Shift</kbd>+矢印キーで 5mm 移動します。キーを押し続けると連続して移動し、キーを離すまでの移動は 1 回で元に戻せます。
- 要素は用紙の右端・下端を越えて配置できます。用紙の外側の部分は PDF に出力されず、そのような要素を選択すると設定パネルに案内が表示されます。左端・上端は越えられず、複数の要素を一緒に動かすと最も外側の要素が端で止まり、要素どうしの間隔は保たれます。
...
キャンバスまたはサイドバーの要素一覧で、<kbd>Shift</kbd> または <kbd>Ctrl</kbd>（macOS では <kbd>⌘</kbd>）を押しながら要素をクリックすると、選択に追加または選択から除外できます。グループは常に 1 つの単位として選択されます。

選択中の要素のいずれかをドラッグすると選択全体が一緒に移動し、グループにまとめることもできます。グループにまとめると、グループ内の要素 1 つを選択しても全体が一緒に選択されます。

複数の要素を選択すると、設定パネルに整列ボタン（左・水平中央・右・上・垂直中央・下）と等間隔ボタン（水平・垂直）が表示されます。整列は選択全体の外側の境界を基準にし、等間隔は要素またはグループが 3 つ以上のときに最初と最後を固定して間隔を揃えます。コマンド 1 回が元に戻す 1 段階です。
...
グリッドを選択すると、<kbd>セルの既定の枠線</kbd> と <kbd>グリッドの枠線</kbd> を個別に設定できます。
セルの既定の枠線は、個別の枠線を指定していないセルに適用されます。グリッドの枠線はグリッド
全体を囲み、既定では表示されません。

セルを選択すると、<kbd>セルの枠線</kbd> を変更できます。セルで指定した項目はセルの既定の枠線
より優先されます。グリッドの枠線を非表示にしても、セル間の境界線は残ります。
...
複数セルのスタイルをまとめて変更するには、セルを 1 つ選択してから次の修飾キーを使います。

| 操作 | 結果 |
|---|---|
| <kbd>Shift</kbd> を押しながら別のセルを選択 | 基準セルから選択したセルまでの長方形範囲を選択 |
| <kbd>Ctrl</kbd>、または macOS の <kbd>⌘</kbd> を押しながらセルを選択 | そのセルを選択に追加、または選択から除外 |
| <kbd>Ctrl</kbd>/<kbd>⌘</kbd> と <kbd>Shift</kbd> を押しながらセルを選択 | 基準セルからの長方形範囲を現在の選択に追加 |

通常のクリック、または Ctrl/⌘ クリックで新たに追加したセルが基準セルになります。基準セルは、
ほかの選択セルより濃い枠線で表示されます。複数選択では、セル名、値の種類、結合、条件付き書式は
変更できず、テキスト、背景、セルの枠線だけをまとめて編集できます。値が異なる項目は
<kbd>混在</kbd> と表示されます。各リセットボタンは、選択したすべてのセルから該当する個別設定を
削除し、グリッドの共通設定を再び継承させます。
...
7. ページ方式を選びます。
8. グリッド設定パネルのタイトル行にある <kbd>出力結果を表示</kbd> で実際の繰り返し結果を確認します。キャンバスの位置はそのまま保たれ、編集操作を行うと自動的に行構造の編集に戻ります。<kbd>Esc</kbd> でも戻れます。
...
実データがあるものの、最初の項目または固定ページのまとまりを最初のページの残り領域に一つも
配置できない場合、空のグリッド断片を残さず、次のページのフロー領域上端から開始します。続きの
ページのフロー領域全体にも収まらない項目や固定ページのまとまりは、配置エラーになります。

`after` 配置ではグリッドの開始位置が縮まるため、実データがなくても同じ移動が起こる場合があります。
それ以外では、固定ページまたは最小表示項目数による空の項目をグリッドの元の位置に維持します。
...
数式の入力欄と計算結果は編集ウィンドウの上部に全幅で配置されます。その下の <kbd>関数</kbd>・<kbd>値</kbd> タブから参照を挿入でき、関数を選ぶと一覧の横に詳細が表示され、挿入するとカーソルは元の位置に戻ります。
...
フォームと JSON モードを切り替えたり、変更せずに適用したりしても、既存のサンプルデータの
キーと値は保持されます。パラメータとして宣言していないキー、`__proto__`・`constructor`・
`toString` のようなキー、不正な形式の既存リスト値も対象です。宣言済みでサンプル値がない
パラメータにだけ、編集用の初期値を追加します。
...
- ヘッダー下の保存に関する案内と <kbd>保存データを削除</kbd> ボタン

実際のアプリケーションでこれらの機能を実装する方法は、[アプリケーション統合ガイド](integration.ja.md) を参照してください。

### ブラウザーに保存されるデータ

デモは作業中のテンプレートと伝票をこのブラウザーに保存し、次に開いたときにそのまま復元します。ヘッダー下の案内がこのことを伝え、作業中は常に表示されます。

同じ案内の <kbd>保存データを削除</kbd> は、デモが自動保存したテンプレート、作成中の伝票、発行済みの伝票、記憶した画面だけを削除します。<kbd>マイテンプレート</kbd> に明示的に保存したテンプレートは残ります。元に戻せないため先に確認を求め、削除後は同梱の最初のテンプレートと空の伝票に戻ります。ダウンロード済みのファイルはそのまま残り、ブラウザーの他の保存項目には触れません。別のテンプレートを開くときも、以前のテンプレートの自動保存伝票が一緒に復元されないよう、作成中の伝票を削除します。

> [!WARNING]
> キーを設定しない場合、デモは自身のソースに公開されたサンプルキーで暗号化します。このキーは何も守りません — デモに実際の個人情報や機密情報を入力せず、共用の端末では使い終わったら保存データを削除してください。

実際のアプリケーションは、データをどこに置くか、利用者がどう削除するかを自分で決めます。[アプリケーション統合ガイド](integration.ja.md) を参照してください。
