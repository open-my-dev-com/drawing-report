...
グループ小計を使う場合は、先に <kbd>詳細設定</kbd> でグループ基準のフィールドを選びます。かんたん設定で追加した行も、行範囲一覧で範囲、役割、表示ページを変更できます。

キャンバスの行番号を選択すると、その行の役割を変更できます。複数行を 1 つの範囲にまとめるには、<kbd>Shift</kbd> を押しながら別の行番号を選択します。同じ操作は右側の行範囲一覧からも行えます。
