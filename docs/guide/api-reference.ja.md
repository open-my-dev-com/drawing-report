...
`dataDependent` は値が変われば解消しうるエラーかどうかを示します。`value` では、同じ箇所の他の被演算子ではなく、変換・検査に実際に失敗した値だけを見ます。`amount / 0` は除数の定数が原因なので保存できず、`amount / quantity` や `amount` が数値でない `amount + 1` は保存できます。エラーメッセージを比較する代わりにこの値を使います。
