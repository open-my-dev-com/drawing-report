...
별도 터미널에서 서버를 계속 실행할 필요는 없습니다. stdio 방식에서는 MCP 클라이언트가 설정의 `command`와 `args`를 사용해 이 서버를 로컬 하위 프로세스로 시작하고, 연결을 종료할 때 함께 종료합니다. 사용자는 실행 파일과 작업 디렉터리를 MCP 설정에 등록하면 됩니다.
...
클라이언트의 stdio MCP 서버 설정에 다음 내용을 추가합니다. 설정 파일의 위치와 최상위 키는 클라이언트마다 다를 수 있습니다.
...
경로는 모두 실제 절대 경로로 바꿔야 합니다. 설정을 저장한 뒤 MCP 클라이언트를 다시 시작하거나 MCP 서버 목록을 새로 고칩니다. `slip_list`, `slip_read` 등의 도구 7개가 보이면 연결된 상태입니다.

### 설정은 어디에 저장되는가

SlipKit MCP 서버 자체는 별도의 설정 파일을 만들지 않습니다. 작업 디렉터리와 로케일은 실행 인자로, 암호화 키는 환경변수로 받습니다. 이 실행 정보를 저장하는 위치는 MCP 클라이언트가 결정합니다.
...
codex mcp add slipkit -- \
  ...
  /absolute/path/to/slip-workspace --locale ko
```
...
```bash
codex mcp add slipkit -- \
  npx -y @omdc-slipkit/mcp /absolute/path/to/slip-workspace --locale ko
```

이 경우에도 작업 디렉터리는 사용자 데이터의 위치이므로 사용 환경에 맞는 경로를 한 번 지정해야 합니다.

### 실행 옵션과 환경변수

| 항목 | 설명 |
|---|---|
| 첫 번째 위치 인자 | 작업 디렉터리. 생략하면 MCP 서버의 현재 디렉터리를 사용합니다. |
...
`SLIPKIT_MCP_KEY`가 설정되면 새로 저장하는 파일은 암호화됩니다. 평문 `.slip` 파일은 그대로 읽을 수 있지만, 암호화된 파일은 일치하는 키가 없으면 읽을 수 없습니다.

### PDF 폰트

폰트 파일 경로나 운영체제 폰트를 설정할 필요는 없습니다. MCP 서버는 `@omdc-slipkit/elements`에 base64로 동봉된 폰트를 메모리에서 읽어 PDF 렌더러에 전달하며, 폰트를 네트워크에서 내려받지 않습니다.
...
요소의 `fontName`을 생략하면 해당 로케일의 기본 폰트를 사용합니다. 직접 지정할 때는 현재 로케일에 등록된 `Pretendard`, `Pretendard-Bold`, `Noto Sans JP` 중 하나를 사용해야 합니다. 일본어 동봉 폰트에는 Bold가 없으므로 `bold: true`를 지정해도 별도 Bold 폰트가 적용되지 않습니다.

현재 CLI에는 사용자 폰트를 추가하는 옵션이 없습니다. 사용자 폰트 지원에는 서버 생성 옵션으로 폰트 공급 함수를 받는 별도 확장이 필요합니다. 개발 저장소에서 실행할 때는 `packages/mcp/dist`만 복사하지 말고 pnpm으로 설치된 workspace 의존성을 유지해야 하며, npm 배포 후에는 `elements` 의존성과 동봉 폰트가 함께 설치됩니다.
...
서버를 직접 구성할 때는 `createSlipMcpServer(options)`를 사용할 수 있습니다. 이 함수는 연결되지 않은 `McpServer`와 `FileSystemStorage`를 반환하며, 전송 연결은 호출자가 담당합니다.

## 문제 해결

| 현상 | 확인할 내용 |
|---|---|
| MCP 도구가 보이지 않음 | MCP 패키지를 빌드했는지, `cli.js`와 작업 디렉터리의 절대 경로가 맞는지, 클라이언트를 재시작했는지 확인합니다. |
| `working directory not found` | 작업 디렉터리를 먼저 만들고 설정의 경로를 확인합니다. |
| 암호화 파일을 읽지 못함 | `SLIPKIT_MCP_KEY`와 `SLIPKIT_MCP_PREVIOUS_KEYS`에 현재 파일을 복호화할 키가 있는지 확인합니다. |
