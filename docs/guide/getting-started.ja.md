...
デモの既定の言語は英語です。アドレスに `?locale=` を付けると、その言語で実行されます。

```text
http://localhost:5173/?locale=ko   # 韓国語
http://localhost:5173/?locale=ja   # 日本語
```

環境変数 `VITE_SLIPKIT_LOCALE` で既定の言語を指定することもできます。アドレスの `?locale=` の値が優先されます。

```bash
VITE_SLIPKIT_LOCALE=ja pnpm demo
```
...
- [ ] 作成した伝票を発行する
- [ ] <kbd>発行済み伝票を見る</kbd> で発行済みの伝票を確認する
- [ ] `.slip` ファイルをダウンロードする
- [ ] PDF ファイルをダウンロードする
