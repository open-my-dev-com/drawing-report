...
このコマンドは MCP パッケージをビルドし、`examples/mcp-demo/workspace` にサンプルテンプレートを準備して、`http://localhost:6274` で Inspector を開きます。接続画面で **Connect** を選び、**Tools** から `slip_list`、`slip_read`、`slip_edit`、`slip_render_pdf` などを呼び出せます。

Inspector で編集・生成したファイルはデモ用作業ディレクトリにだけ保存され、Git から除外されます。サンプルを初期状態に戻すには Inspector を終了し、次のコマンドを実行します。

```bash
pnpm demo:mcp:reset
```

そのまま使える入力例は [`examples/mcp-demo`](../../examples/mcp-demo) を参照してください。
...
設定ファイルは任意の場所に配置できます。`rootDir` とフォントの相対パスは、設定ファイルを置いたディレクトリを基準に解決されます。`~` はホームディレクトリに展開されないため、絶対パスまたは正しい相対パスを使用してください。シンボリックリンクはストレージ操作から除外されますが、作成済みのハードリンクは通常のファイルと区別できません。機密ファイルへのハードリンクを作業ディレクトリに置かないでください。
...
| PDF リンク | 設定ファイルの `httpPort`。省略時は無効 |
...
| `--help`, `-h` | CLI の使用方法を stdout に出力し、設定の読み込みやサーバーの起動を行わずに終了します。 |
| `--version`, `-v` | npm パッケージのバージョンを stdout に出力し、サーバーを起動せずに終了します。 |
...
不明なオプション、オプション値の不足、2 つ以上の作業ディレクトリ位置引数は使用方法エラーです。CLI は原因と `--help` の案内を stderr に出力し、終了コード 2 を返します。設定およびサーバー起動時のエラーは終了コード 1 を返します。サーバー実行中は stdout を MCP プロトコルメッセージ専用とし、起動情報は stderr に出力します。
...
読み取り応答の base64 画像データはサイズ表示に置き換えられます。置き換えるのは `data:<種類>/<下位種類>;base64,` のヘッダーを持つ埋め込み値だけで、`data:text/plain,hello` のように payload をそのまま含む値や `data:` で始まるだけの一般的な文字列を含め、それ以外の文字列はそのまま返します。`.slip` ファイル内の画像形式は base64 データ URL です。MCP 経由で画像を追加するときは、`set_image` 操作に作業ディレクトリ内のファイルパスを渡します。サーバーがファイルを読み取り、base64 アセットを作成します。

画像データの検証は画像として宣言された値だけに適用します。画像要素が参照するパラメータ、`valueType: 'image'` のパラメータと同じ型のリスト下位フィールドが対象です。`data:` で始まる未定義キーを含むその他の業務データ文字列はそのまま保持します。
...
- すべての入出力パスは、起動時に指定した作業ディレクトリ内に制限されます。読み取り、書き込み、削除、画像、PDF、HTTP アクセスでは、既存パスとシンボリックリンク先の実パスを確認します。
- `rootDir` は実パスに正規化します。対象自体がシンボリックリンクなら拒否し、`slip_list` はリンクファイルとリンクされたディレクトリを除外します。
...
- PDF の `outPath` は `.pdf` で終わる必要があります。既存の対象が PDF の場合だけレンダリング結果で置き換えます。
- `set_image` はファイルごとに最大 2 MiB の PNG と JPEG のみをサポートし、ファイルシグネチャとメディアタイプを確認します。
