...
[English](server-integration.md) · [日本語](server-integration.ja.md)
...
> [!NOTE]
> Core API 자체의 사용법은 [Core 사용 가이드](core.ko.md)를 참고하세요.
...
폰트 이름과 굵기 연결 방법은 [환경 설정 가이드](configuration.ko.md)를 참고하세요.
...
export class SlipIssuanceService {
  ...
  async issue(
    ...
    if (template.kind !== 'template') {
      // 클라이언트 요청이 아니라 서버 저장 데이터의 문제입니다
...
- [ ] 발행 기준 양식을 서버의 신뢰할 수 있는 저장소에서 읽습니다.
- [ ] 발행 전에 외부 URL 이미지를 내장 데이터로 변환합니다.
- [ ] 발행 상태로 변경한 전표를 다시 검증합니다.
- [ ] 전표 전체와 필요한 PDF를 함께 보관합니다.
- [ ] 발행 실패와 부분 저장 상태를 복구할 수 있습니다.
- [ ] 동시 PDF 생성 수와 요청 본문 크기를 제한합니다.
- [ ] 인증, 권한, 감사 기록과 보존 정책을 서버에서 관리합니다.
- [ ] 발행 당시의 SlipKit 버전과 렌더 설정을 기록합니다.
