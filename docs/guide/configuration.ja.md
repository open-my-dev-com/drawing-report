...
日本語の既定フォントは、一般的なかな、漢字、ラテン文字を含むサブセットです。Pretendard はハングルとラテン文字を収録していますが一般的な漢字は収録しておらず、同梱の Noto Sans JP サブセットはハングル音節を収録していません。複数言語を混在させる文書では、使用するすべての文字を収録したフォントを供給するか、要素ごとにフォントを分けて指定してください。同梱範囲外の文字や太字の日本語にも独自フォントが必要です。
