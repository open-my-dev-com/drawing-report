...
`locale` を省略するか、サポートしない値を渡すと、韓国語を使います。
...
> [!IMPORTANT]
> 現在同梱されている取引明細書と請求書のプリセットは韓国語のテンプレートです。
> `locale="en"` または `locale="ja"` を使っても、プリセット内のタイトルや表の項目は自動的には翻訳されません。
> 別の言語のテンプレートが必要なら、その言語で作成したプリセットを別途供給してください。
...
| `locale` | `FORMAT_NUMBER` などの数式フォーマット関数に使う BCP-47 ロケール |
...
| Core `locale` | `'ko-KR'`, `'en-US'`, `'ja-JP'` | 数値と日付の数式フォーマット |
