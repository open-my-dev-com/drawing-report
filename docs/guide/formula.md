...
SlipKit 수식은 양식에 입력된 값을 계산하거나 표시 형식으로 변환할 때 사용합니다.

필드, 그리드 셀, 바코드에서 수식을 사용할 수 있으며 다음과 같은 작업을 처리할 수 있습니다.
...
자세한 화면 조작 방법은 [양식 디자이너 사용 가이드](designer.md#7-수식-작성하기)를 확인하세요.
...
표시 결과는 수식 평가 컨텍스트의 `locale` 설정을 따릅니다. `locale`을 지정하지 않으면 `ko-KR`을 사용합니다.
...
디자이너에서는 수식 편집 창에 문법 오류와 샘플 값을 이용한 계산 결과가 표시됩니다.

PDF를 생성할 때 수식 계산이 실패하면 어떤 필드·그리드 셀·바코드에서 실패했는지 포함한 렌더링 오류가 발생합니다.
