...
> [!NOTE]
> 컴포넌트 이벤트와 자동 저장 흐름은 [애플리케이션 통합 가이드](integration.ko.md)를, 각 설정 타입의 전체 필드는 [API 참조](api-reference.ko.md)를 확인하세요.
>
> 로컬 MCP 서버의 `slipkit-mcp.json` 설정은 [MCP 사용 가이드](mcp.ko.md)에서 별도로 설명합니다.
...
일본어 기본 폰트는 일반적인 가나, 한자와 라틴 문자를 포함하는 서브셋입니다. Pretendard는 한글과 라틴 문자를 지원하지만 일반적인 한자를 포함하지 않고, 동봉 Noto Sans JP 서브셋은 한글 음절을 포함하지 않습니다. 여러 언어가 섞인 문서에는 사용하는 모든 문자를 지원하는 폰트를 제공하거나 요소별로 폰트를 나누어 지정하세요. 동봉 범위에 없는 글자나 굵은 일본어 글자에도 사용자 폰트가 필요합니다.
...
> 조회에 실패하면 다음 호출에서 제공 함수를 다시 실행합니다.
...
> 애플리케이션에서 실제로 사용하는 문자와 굵기를 포함하는 폰트만 제공하세요.
...
자동 저장과 서버 저장소 연결은 [애플리케이션 통합 가이드](integration.ko.md)를 참고하세요.
...
Core 사용 흐름과 PDF 생성 방법은 [Core 사용 가이드](core.ko.md)를 참고하세요.
...
- 사용자 폰트를 제공하면서 필요한 기본 폰트가 자동으로 추가된다고 가정
...
- [ ] 출력할 문자와 스타일에 필요한 폰트를 제공했습니다.
- [ ] 폰트 조회 결과를 재사용하도록 구성했습니다.
