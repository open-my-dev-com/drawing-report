...
일본어 기본 폰트는 일반적인 가나, 한자와 라틴 문자를 포함하는 서브셋입니다. Pretendard는 한글과 라틴 문자를 지원하지만 일반적인 한자를 포함하지 않고, 동봉 Noto Sans JP 서브셋은 한글 음절을 포함하지 않습니다. 여러 언어가 섞인 문서에는 사용하는 모든 문자를 지원하는 폰트를 공급하거나 요소별로 폰트를 나누어 지정하세요. 동봉 범위에 없는 글자나 굵은 일본어 글자에도 사용자 폰트가 필요합니다.
