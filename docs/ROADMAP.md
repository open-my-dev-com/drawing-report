...
최종 갱신: 2026-08-20 (C-10 표 내부 편집 완료 — 셀 클릭 입력·병합·동적 표 열 편집. 다음 작업 = C-11 도형 그리기·글자 스타일 UI)
...
11. `feat/elements-designer-shape-text` — 도형 그리기·글자 스타일(굵게·밑줄·취소선) UI + **도형 선택 메뉴(2026-08-20 확정): 도형 버튼을 누르면 프리셋 메뉴처럼 사각형·타원·삼각형·오각형·육각형 등 선택지가 펼쳐지고 고른 도형으로 그리기** + **테두리 편집 개편(2026-08-20 확정): 굵기는 없음+정해진 단계만 select로(숫자 옆 굵기 미리보기 선 표시), 형태·모서리 반경 선택, 속성 패널 스타일을 텍스트/배경/테두리 그룹으로 재구성** + **선 전용 편집(2026-08-20 확정): 선택 표시를 상자 대신 선 하이라이트 + 양 끝점 핸들 2개로, 생성은 드래그와 두 번 클릭(첫 클릭=시작점 → 반투명 미리보기 선 → 둘째 클릭=끝점, Esc 취소) 공존**
