...
9. ~~`feat/core-format-0-2` — 스키마 개정 (ADR-032)~~ → **완료**: 동적 표 열 구조(`columns`: 물리 키·제목·너비 분리), 선 방향(`lineDirection` — 사선 지원, 캔버스 선 미표시 문제 함께 해소·실브라우저 확인), 타원·삼각형, 굵게·밑줄·취소선, 테두리 형태 렌더 규칙(직선만 분해 렌더 — PDF 직접 확인: svg dasharray 미지원)·사각형 모서리 반경, `sampleValues`, 요소 그룹 필드, 바인딩 정의부(물리명 `key`+논리명 `label`, 프리셋 camelCase 전환). SPEC·마이그레이션(0.1.1→0.2.0)·JSON Schema 동시 갱신, 캔버스·PDF 동일 표시 직접 확인
