...
최종 갱신: 2026-08-20 (C-11 도형·선·글자 스타일·테두리 편집 완료 — C 묶음 종료. 다음 작업 = D-12 수식 편집 모달 + 동적 표 열 편집 모달)
...
12. `feat/elements-designer-formula-modal` — 수식 편집 모달: 함수 29종 분류·설명·클릭 삽입, 바인딩 목록, 실시간 문법 검사, 결과 미리 계산 + **모달 틀 첫 도입과 함께 동적 표 열 편집 모달 추가(2026-08-20 확정): 열 전체 관리(데이터 키·추가·삭제·이후 순서 변경 등)는 모달에서, 속성 패널에는 제목·너비 빠른 수정만 유지**
