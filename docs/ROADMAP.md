...
9. ~~`feat/core-format-0-2` — 스키마 개정 (ADR-032)~~ → **완료**: 동적 표 열 구조(`columns`: 물리 키·제목·너비 분리), **도형을 독립 요소 타입 4종(선·사각형·타원·삼각형)으로 분리(요소 9종)** — 종류별로 의미 있는 스타일만 갖고, 툴바도 4개 도구로 분리, 선 도구는 드래그 방향 그대로 사선 생성, 캔버스에서 선은 상자 테두리 없이 선만 표시(선 미표시 문제 함께 해소·실브라우저 확인). 굵게·밑줄·취소선, 테두리 형태 렌더 규칙(직선만 분해 렌더 — PDF 직접 확인: svg dasharray 미지원)·사각형 모서리 반경, `sampleValues`, 요소 그룹 필드, 바인딩 정의부(물리명 `key`+논리명 `label`, 프리셋 camelCase 전환). SPEC·마이그레이션(0.1.1→0.2.0)·JSON Schema 동시 갱신, 캔버스·PDF 동일 표시 직접 확인
10. `feat/elements-designer-table-edit` — 표 내부 편집: 고정 그리드 행·열·셀 텍스트·병합, 동적 표 열 편집 (콤마 나열 입력 폐지)
11. `feat/elements-designer-shape-text` — 도형 그리기·글자 스타일(굵게·밑줄·취소선) UI + **테두리 편집 개편(2026-08-20 확정): 굵기는 없음+정해진 단계만 select로(숫자 옆 굵기 미리보기 선 표시), 형태·모서리 반경 선택, 속성 패널 스타일을 텍스트/배경/테두리 그룹으로 재구성** + **선 전용 편집(2026-08-20 확정): 선택 표시를 상자 대신 선 하이라이트 + 양 끝점 핸들 2개로, 생성은 드래그와 두 번 클릭(첫 클릭=시작점 → 반투명 미리보기 선 → 둘째 클릭=끝점, Esc 취소) 공존**
