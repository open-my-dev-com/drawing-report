...
13. ~~`feat/elements-designer-sample-preview` — 샘플 데이터 편집 + 채운 미리보기 + 사이드바 바인딩 등록·삭제·논리명 편집~~ → **완료**: 샘플 데이터 모달 — 바인딩마다 값 입력(동적 표 바인딩은 그 표의 열 구조대로 행 추가·삭제·셀 편집), 숫자 표기는 수로 저장해 수식 계산과 맞물림. 샘플 값이 있으면 미리보기가 그 값으로 채운 전표를 렌더(수식도 계산, 파일은 양식 그대로). 사이드바 바인딩 섹션에서 등록(+, 물리명·논리명)·논리명 인라인 편집(정의부에 없던 사용처 키도 항목 생성)·정의부 삭제(요소 사용처는 목록에 유지). 수식 모달 바인딩 칩에도 등록 바인딩이 바로 나타남
