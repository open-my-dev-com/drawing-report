...
- **PDF 렌더러 완료**: `.slip` → pdfme 변환 계층(요소 6종 매핑, 고정 격자는 선·사각형·텍스트 분해, 표 스타일 기본값 병합) + 렌더러 인터페이스 은닉(공개 API에 pdfme 노출 0건) + field 수식 평가 연동 + 자동 페이지 분할 확인 (ADR-011/016)
...
4. ~~`feat/core-pdf-renderer` — 우리 포맷 → pdfme 템플릿 변환 계층 + 렌더러 인터페이스 은닉 (ADR-016)~~ → **완료**
