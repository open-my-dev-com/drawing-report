...
최종 갱신: 2026-08-20 (B 완료 후 화면 확인 반영 — 사이드바 썸네일 표시 수정, 바인딩 정의부·테두리 편집 확정 기록. 다음 작업 = C-9 파일 포맷 0.2.0)
...
9. `feat/core-format-0-2` — 스키마 개정: 동적 표 열 구조(키·제목·너비 분리), **선 시작·끝점(자유선·사선 — 필수, 2026-08-19 확정. A-1의 캔버스 선 표시가 그려지지 않는 문제도 이 방식으로 함께 해소)**, 타원·삼각형, 굵게·밑줄·취소선, **테두리 형태(실선·파선·점선)·모서리 반경(radius) 필드(2026-08-20 확정 — PDF 변환이 실제로 그리는 범위를 확인해 지원되는 것만 노출, 화면·PDF 일치 원칙)**, `sampleValues`, 요소 그룹 필드, **바인딩 정의부(2026-08-20 확정): 물리명 `key` + 논리명 `label` 목록 — 화면에는 논리명, 파일·수식·백엔드 연동은 물리명. 동봉 프리셋 바인딩도 camelCase 물리명 + 한국어 논리명으로 전환**. SPEC·마이그레이션·JSON Schema 동시 갱신
10. `feat/elements-designer-table-edit` — 표 내부 편집: 고정 그리드 행·열·셀 텍스트·병합, 동적 표 열 편집 (콤마 나열 입력 폐지)
11. `feat/elements-designer-shape-text` — 도형 그리기(선·타원·삼각형)·글자 스타일(굵게·밑줄·취소선) UI + **테두리 편집 개편(2026-08-20 확정): 굵기는 없음+정해진 단계만 select로(숫자 옆 굵기 미리보기 선 표시), 형태·모서리 반경 선택, 속성 패널 스타일을 텍스트/배경/테두리 그룹으로 재구성**
