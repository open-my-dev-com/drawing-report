...
최종 갱신: 2026-08-20 (**D 묶음 전부 완료** — D-16 데모 사용성까지. 다음 = 사용자 확인·피드백 후 E 묶음(고급 편집) 진행)
...
20. `feat/core-table-nesting` — 표 중첩: 스키마 0.3.0 범위 + 변환 + 디자이너 편집
    - 0.3.0 후보 추가(2026-08-20 확정): **고정 그리드 셀 안 수식·바인딩** — 셀 `content`에 수식·바인딩을
      허용해 별도 필드 요소 없이 표 안에서 합계 등을 계산. 착수 시 19·20과 함께 범위 확정
