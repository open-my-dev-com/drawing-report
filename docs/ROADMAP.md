...
8. `feat/core-format-0-2` — 스키마 개정 1회: 동적 표 열 구조(키·제목·너비 분리), 선 시작·끝점, 타원·삼각형, 굵게·밑줄·취소선, `sampleValues`. SPEC·마이그레이션·JSON Schema 동시 갱신
...
## v3 후보 (미정리 — 착수 전 사용자와 정리)

- **MCP 제공 + AI 전표 자동 생성** (2026-08-19 사용자 지정으로 v3 배정. 사전 리서치 완료 — 실현성 높음)
- VLOOKUP류 수식 함수 (ADR-017), 요소 그룹화·표 중첩 (ADR-020), 동적 표 셀 병합, 동봉 프리셋 확충
