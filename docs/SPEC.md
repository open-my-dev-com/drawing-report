...
- 최종 갱신: 2026-08-23
- 근거 ADR: 001, 007, 008, 011, 014, 018, 019, 020, 022, 037, 046
...
| `bindings` | — | 바인딩 정의부 (ADR-032): `{ key, label? }` 배열. **물리명 `key`**는 파일·수식·백엔드 연동에, **논리명 `label`**은 화면 표시에 쓴다. `key`는 목록 안에서 유일. 정의부는 보조 정보이며 요소가 미등록 키를 쓰는 것도 허용된다. **값 종류 `valueType`**(`text`\|`number`\|`date`\|`boolean`\|`image`\|`list`, 선택)도 둘 수 있다 — 작성폼 입력 방식과 쓸 수 있는 함수를 가리는 데 쓰며 생략하면 글자로 다룬다 |
