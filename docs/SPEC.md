...
```json
{ "schemaVersion": "0.3.0", "kind": "template" }
```

- `schemaVersion`: 이 문서가 따르는 스키마 버전. `MAJOR.MINOR.PATCH` semver 형식 필수.
- 현재 버전은 **`0.3.0`**이다. 제품 v1 안정 릴리스 시점에 `1.0.0`으로 확정한다.
...
위쪽 11개는 구조 검증(§9-1) 대상이라 동봉 JSON Schema에도 반영된다. 수식 길이·중첩과
...
## 5. 요소 10종 (ADR-020/032/037)

모든 요소의 공통 필드:

| 필드 | 필수 | 내용 |
|---|---|---|
| `type` | ✅ | 요소 종류 판별자 (아래 10종) |
| `id` | ✅ | 문서 내 유일 id |
| `name` | ✅ | 디자이너에 표시되는 이름 |
...
| 필드 | 필수 | 내용 |
|---|---|---|
| `columns` | ✅ | 열 정의 배열 (1개 이상, ADR-032): `{ key, title, widthPercentage }`. `key` = 행 객체에서 값을 읽는 물리 키(목록 안 유일), `title` = 헤더에 표시하는 제목, `widthPercentage` 합 100 |
| `repeatHead` | ✅ | 페이지 분할 시 헤더 반복 여부 |
...
### 5.7 `grid` — 그리드 (0.3.0, ADR-037)

고정 틀과 반복 목록을 **하나로 다루는 그리드**다. 행 범위 하나를 반복 구간으로 지정하면
그 구간이 데이터 배열 길이만큼 복제된다. 반복 구간을 두지 않으면 고정 틀로 쓴다.

`fixedGrid`·`dynamicTable`을 대신하는 요소이며, 두 요소는 당분간 함께 유효하다
(마이그레이션은 ADR-037의 3단계에서 수행한다).

| 필드 | 필수 | 내용 |
|---|---|---|
| `columns` | ✅ | 열 정의 배열(1개 이상): `{ width }` — 너비 **mm** |
...
셀(병합 범위 포함)은 그리드를 벗어나거나 서로 겹치면 안 된다. 셀별 테두리 규칙은 `fixedGrid`와
같다 (§5.2, ADR-033).
...
```json
{ "schemaVersion": "0.3.0", "kind": "template", "template": { ...양식 본문(§4)... } }
```
