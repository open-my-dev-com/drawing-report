...
```json
{ "schemaVersion": "0.4.0", "kind": "template" }
```

- `schemaVersion`: 이 문서가 따르는 스키마 버전. `MAJOR.MINOR.PATCH` semver 형식 필수.
- 현재 버전은 **`0.4.0`**이다. 제품 v1 안정 릴리스 시점에 `1.0.0`으로 확정한다.
...
## 5. 요소 8종 (ADR-020/032/037)

모든 요소의 공통 필드:

| 필드 | 필수 | 내용 |
|---|---|---|
| `type` | ✅ | 요소 종류 판별자 (아래 8종) |
| `id` | ✅ | 문서 내 유일 id |
| `name` | ✅ | 디자이너에 표시되는 이름 |
...
```json
{ "schemaVersion": "0.4.0", "kind": "template", "template": { ...양식 본문(§4)... } }
```
