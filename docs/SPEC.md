...
### 5.2 `fixedGrid` — 고정 격자 표

행 수가 고정된 격자 틀(공급자 정보란 등). **셀 병합을 지원**한다 (ADR-020).
...
색 스타일·글꼴 필드 선택. 셀(병합 범위 포함)은 격자를 **벗어나거나 서로 겹치면 안 된다**.
...
- 서명 키 관리·서명 주체는 호스트 책임이다 (ADR-004와 정합 — 통상 호스트 서버가 서명).
...
2. **정합 검증**: JSON Schema로 표현되지 않는 교차 필드 규칙 — 비율 합 100,
   격자 셀 범위·겹침, `asset://` 참조 해소, 요소/에셋 id 유일성, §7.1 발행 규칙.
