// 호스트 폰트가 있으면 동봉 폰트 청크를 읽지 않는지 확인합니다.
// 청크 평가 횟수가 0인 상태에서 시작해야 하므로 `default-fonts-loading.test.ts`와 파일을 나눕니다.
...
describe('호스트 폰트가 있을 때의 동봉 폰트 청크', () => {
  it('getFonts가 비어 있지 않은 목록을 주면 그 목록을 그대로 반환하고 청크를 읽지 않는다', async () => {
    ...
    // createSlipKit 인스턴스도 같은 방식으로 조회 결과를 그대로 전달합니다.
