 ...
 *
 * @remarks
 * 문법 오류와 샘플 값으로 계산할 수 없는 경우를 구분합니다. 앞은 어떤 데이터에서도
 * 틀린 수식이라 적용을 막고, 뒤는 실제 전표 값에서는 계산될 수 있어 적용을 허용합니다.
 */

import { parseFormula, type FormulaContext, type FormulaValue } from '@omdc-slipkit/core';

/** 수식·조건식 검사 결과의 종류 */
export type FormulaCheckStatus =
  /** 비어 있음 */
  | 'empty'
  /** 문법이 깨져 어떤 데이터에서도 계산할 수 없음 */
  | 'syntax-error'
  /** 샘플 값으로 계산에 성공 */
  | 'ok'
  /** 조건식인데 결과가 논리값이 아님 */
  | 'not-boolean'
  /** 문법은 맞지만 샘플 값으로는 계산할 수 없음 */
  | 'not-computable'
  /** 편집 대상이 지워졌거나 바뀜 */
  | 'target-changed';
