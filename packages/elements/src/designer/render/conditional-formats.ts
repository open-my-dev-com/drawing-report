...
export function conditionalFormatsSection(
  ...
  targetOf: (index: number) => FormulaTarget,
  ariaPrefix = '',
) {
  const s = kit.s;
  const list = rules ?? [];
          ...
          <div class="prop-row">
            ...
            <input .value=${live(rule.condition)}
              ...
              @change=${(e: Event) => {
                ...
                if (!kit.acceptFormula(targetOf(index), value, `${keyPrefix}-cond-${index}`)) return;
                change((next) => { next[index]!.condition = value.trim(); });
              }}>
            <button class="row-btn"
              title=${s.formulaModalTitle} aria-label="${name}: ${s.formulaModalTitle}"
              @click=${() => kit.openFormulaModal(targetOf(index))}
