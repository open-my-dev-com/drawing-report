/**
 * 동봉된 Pretendard와 Noto Sans JP를 지연 불러옵니다.
 *
 * `slipkit.getFonts`가 없을 때 사용하며, 로케일별로 만든 등록 목록을 재사용합니다.
 *
 * Pretendard와 Noto Sans JP를 모두 등록하므로 로케일과 관계없이 `fontName`으로 선택할 수 있습니다.
 * 로케일은 `fontName`을 지정하지 않은 요소에 적용할 대체 폰트만 결정합니다.
...
/** 지정한 폰트에만 `fallback` 속성을 설정한 새 목록을 만듭니다. */
...
/**
 * 동봉 기본 폰트 목록을 불러옵니다. Pretendard와 Noto Sans JP를 모두 포함합니다.
 *
 * @param locale - 대체 폰트를 선택할 로케일. `'ja'`이면 Noto Sans JP, 그 밖에는 Pretendard를 사용합니다.
