/**
 * 호스트가 SlipKit 컴포넌트에 디자이너 전용 정보를 제공하는 인터페이스입니다.
 *
 * @remarks
 * 사용자 폰트와 로케일은 `createSlipKit` 인스턴스(`slipkit` 속성)가 제공하므로 여기서 받지 않습니다.
 */
import {
  renderSlipToPdf,
  type BarcodeKind,
...
/** core의 렌더링 폰트 타입을 다시 내보냅니다. */
export type { SlipFont };

/**
 * 디자이너의 용지 선택 목록에 표시할 크기 정보입니다.
 */
export interface PaperSize {
  /** 용지 선택기에 표시할 이름입니다. 예: A4, 라벨 100×150 */
  name: string;
  /** 너비(mm)입니다. */
  width: number;
  /** 높이(mm)입니다. */
...
/**
 * 바코드 종류와 용지 정보를 제공하는 디자이너 설정입니다.
 */
export interface SlipDesignerSettings {
  /**
   * 디자이너에 표시할 바코드 종류를 반환합니다.
   *
   * @remarks
   * 생략하거나 빈 배열을 반환하면 지원하는 바코드 종류를 모두 표시합니다.
   *
   * @returns 표시할 바코드 종류 목록. 빈 배열이면 지원하는 종류를 모두 표시합니다.
   */
  getBarcodeKinds?(): BarcodeKind[] | Promise<BarcodeKind[]>;
  /**
   * 기본 용지 뒤에 추가할 용지 목록을 반환합니다.
   *
   * @returns 용지 후보 목록(동기 또는 Promise)
   */
  getPaperSizes?(): PaperSize[] | Promise<PaperSize[]>;
  /**
   * 디자이너에서 직접 입력한 용지 크기를 저장합니다.
   *
...
/**
 * SlipKit 인스턴스에 설정된 렌더링 폰트를 가져옵니다.
 * `getFonts`가 없거나 빈 목록을 반환하면 로케일에 맞는 동봉 기본 폰트를 사용합니다.
 *
 * @param slipkit - 공통 설정 인스턴스(생략 가능)
 * @param locale - 기본 폰트를 선택할 로케일. 지역 코드가 포함되면 언어 코드만 사용합니다.
...
/**
 * `.slip` 파일을 컴포넌트 미리보기용 PDF로 렌더링합니다.
 * 사용자 폰트를 실제로 제공한 SlipKit 인스턴스는 해당 인스턴스로 렌더링합니다.
 * `getFonts`가 없거나 빈 목록을 반환하면 렌더 로케일에 맞는 동봉 기본 폰트를 사용합니다.
 * 디자이너 캔버스도 같은 기준으로 폰트를 선택합니다.
 *
 * @param slipkit - 공통 설정 인스턴스(생략 가능)
