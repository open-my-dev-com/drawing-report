...
/** 표시할 오류 종류입니다. 문구는 렌더링 시점의 로케일에 맞게 선택합니다. */
type ViewerError = 'parseError' | 'renderError';

/**
 * `.slip` 양식 또는 전표를 PDF로 렌더링해 표시하는 웹 컴포넌트입니다.
 */
export class SlipViewer extends LitElement {
  ...
  /**
   * 폰트·로케일 공통 설정 인스턴스. PDF 렌더링은 이 인스턴스의 설정을 사용합니다.
  ...
  override connectedCallback(): void {
    ...
    // 분리됐다 다시 연결되면 분리 중 버린 결과 대신 현재 src로 미리보기를 복구합니다.
  ...
  // 렌더 시작은 갱신 전에 처리해 같은 갱신에 로딩 상태가 반영되도록 합니다.
  // 갱신이 끝난 뒤 동기적으로 상태를 바꾸면 Lit이 중복 갱신을 경고합니다.
  ...
  private _revokePdfUrl(): void {
    // 분리되거나 소스가 바뀐 뒤 완료된 렌더 결과는 사용하지 않습니다.
  ...
  private async _renderPdf(): Promise<void> {
    ...
    try {
      ...
      // 연결이 끊겼거나 새 렌더링이 시작되었으면 결과를 버리고 Blob URL도 만들지 않습니다.
