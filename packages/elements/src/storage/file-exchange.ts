/**
 * 브라우저의 다운로드와 파일 선택 대화상자로 `.slip` 파일을 주고받습니다.
 *
...
/** `SlipFileExchange`를 생성할 때 필요한 설정입니다. */
export interface SlipFileExchangeOptions {
  /**
   * 내려받는 파일을 암호화할지 여부. 키와 로케일은 SlipKit 인스턴스의 설정을 씁니다.
   * 열기는 이 값과 무관하게 암호화 봉투를 자동 감지해 설정된 키로 풉니다.
   *
   * @defaultValue false
   */
...
/**
 * `.slip` 파일을 다운로드로 내보내고 파일 선택 대화상자에서 불러옵니다.
 */
export class SlipFileExchange {
  ...
  /**
   * `.slip` 파일을 브라우저 다운로드로 내려받습니다.
   *
   * @param name - 파일 이름 (`.slip` 확장자는 없으면 붙입니다)
  ...
  /**
   * 파일 선택 대화상자에서 선택한 `.slip` 파일을 읽습니다.
   *
