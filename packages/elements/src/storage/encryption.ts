/**
 * 저장소가 `.slip` 파일과 문자열을 서로 변환할 때 사용하는 공통 처리입니다.
 *
 * 암호화와 복호화는 호스트가 만든 {@link SlipKit} 인스턴스를 그대로 사용합니다.
 * 키와 이전 키 목록은 `createSlipKit` 설정 한 곳에서만 관리합니다.
 */
import {
  isEncryptedSlipFile,
  parseSlipFile,
 ...
 * @param encryptOnSave - true면 설정된 키로 암호화 봉투를 만듭니다. false면 평문 JSON
...
/**
 * 저장소에서 읽은 문자열을 `.slip` 파일로 변환합니다.
 * 암호화 봉투는 저장 정책과 무관하게 설정된 현재 키와 이전 키로 복호화를 시도하고,
 * 일반 `.slip` JSON은 바로 파싱합니다.
 *
 * @param slipkit - 공통 설정을 가진 SlipKit 인스턴스
