/**
 * `slipkit-mcp.json` 설정 파일을 읽습니다.
 *
 * 저장소 경로, 로케일, 사용자 지정 폰트와 암호화 키 환경변수 이름을 설정 파일에서 읽습니다.
 *
 * `--config` 인자, `SLIPKIT_MCP_CONFIG` 환경변수, 작업 디렉터리의
 * `slipkit-mcp.json` 순서로 설정 파일을 찾습니다. 설정 파일이 없으면 기본값으로 동작합니다.
 * 작업 디렉터리는 CLI 인자가, 로케일은 CLI 인자와 환경변수가 설정 파일보다 우선합니다.
 */
import { readFile, stat } from 'node:fs/promises';
import path from 'node:path';
import { z } from 'zod';
import type { SlipFont } from '@omdc-slipkit/core';
import type { SlipMcpServerOptions } from './server.js';

/** 기본 설정 파일 이름입니다. */
export const CONFIG_FILE_NAME = 'slipkit-mcp.json';

/** 암호화 키를 읽을 기본 환경변수 이름입니다. */
export const DEFAULT_KEY_ENV = 'SLIPKIT_MCP_KEY';

/** 쉼표로 구분한 이전 키 목록을 읽을 기본 환경변수 이름입니다. */
export const DEFAULT_PREVIOUS_KEYS_ENV = 'SLIPKIT_MCP_PREVIOUS_KEYS';

/** 설정을 읽거나 적용하지 못했을 때 발생하는 오류입니다. 메시지는 서버 시작 실패 안내에 사용합니다. */
export class SlipMcpConfigError extends Error {}

const fontEntrySchema = z.strictObject({
  /** 요소의 `fontName`에서 참조하는 폰트 이름입니다. */
  name: z.string().min(1),
  /** 폰트 파일(ttf·otf)의 경로입니다. 설정 파일 위치를 기준으로 한 상대 경로입니다. */
  path: z.string().min(1),
  /** 대체 폰트로 사용할지 정합니다. 하나만 지정할 수 있습니다. */
  fallback: z.boolean().optional(),
});

const configSchema = z.strictObject({
  /** `.slip` 파일을 읽고 쓸 디렉터리입니다. 설정 파일 위치를 기준으로 한 상대 경로입니다. */
  rootDir: z.string().min(1).optional(),
  /** 오류 메시지 언어(`ko`, `en`, `ja`)입니다. */
  locale: z.string().min(1).optional(),
  /** PDF 렌더링에 사용할 사용자 지정 폰트입니다. 생략하면 동봉 폰트를 사용합니다. */
  fonts: z.array(fontEntrySchema).min(1).optional(),
  /** PDF 링크 서버 포트입니다. 지정하면 렌더링한 PDF를 127.0.0.1 URL로 제공합니다. */
  httpPort: z.number().int().min(1).max(65535).optional(),
  /** 암호화 키를 읽을 환경변수 이름입니다. 키 값은 설정 파일에 적지 않습니다. */
  encryption: z
    .strictObject({
      keyEnv: z.string().min(1).optional(),
      previousKeysEnv: z.string().min(1).optional(),
    })
    .optional(),
});

/** `slipkit-mcp.json`의 내용입니다. */
export type SlipMcpConfig = z.infer<typeof configSchema>;

/**
 * 설정 파일을 읽고 형식을 검증합니다.
 *
 * @param filePath - 설정 파일 경로입니다.
 * @returns 검증된 설정을 반환합니다.
 * @throws 파일을 읽지 못하거나 JSON 또는 설정 형식이 잘못되면 {@link SlipMcpConfigError}가 발생합니다.
 */
export async function readConfigFile(filePath: string): Promise<SlipMcpConfig> {
  let textContent: string;
  try {
    textContent = await readFile(filePath, 'utf8');
  } catch {
    throw new SlipMcpConfigError(`Could not read the config file: ${filePath}`);
  }
  let raw: unknown;
  try {
    raw = JSON.parse(textContent);
  } catch (error) {
    throw new SlipMcpConfigError(
      `The config file is not valid JSON (${filePath}): ${error instanceof Error ? error.message : String(error)}`,
    );
  }
  const parsed = configSchema.safeParse(raw);
  if (!parsed.success) {
    const issues = parsed.error.issues
      .map((issue) => `${issue.path.join('.') || '(root)'}: ${issue.message}`)
      .join('; ');
    throw new SlipMcpConfigError(`Invalid config file (${filePath}): ${issues}`);
  }
  return parsed.data;
}

/**
 * 설정의 폰트 항목을 읽어 렌더링용 폰트 목록으로 만듭니다.
 *
 * @param entries - 설정 파일의 `fonts` 항목입니다.
 * @param baseDir - 상대 경로의 기준 디렉터리인 설정 파일 위치입니다.
 * @returns 파일에서 읽은 폰트 목록을 반환합니다.
 * @throws 폰트 파일이 없거나 대체 폰트가 둘 이상이면 {@link SlipMcpConfigError}가 발생합니다.
 */
export async function loadConfigFonts(
  entries: NonNullable<SlipMcpConfig['fonts']>,
  baseDir: string,
): Promise<SlipFont[]> {
  if (entries.filter((entry) => entry.fallback === true).length > 1) {
    throw new SlipMcpConfigError('Only one font can set "fallback": true.');
  }
  const fonts: SlipFont[] = [];
  for (const entry of entries) {
    const abs = path.resolve(baseDir, entry.path);
    let data: Buffer;
    try {
      data = await readFile(abs);
    } catch {
      throw new SlipMcpConfigError(`Font file for "${entry.name}" not found: ${abs}`);
    }
    fonts.push({
      name: entry.name,
      data: new Uint8Array(data),
      ...(entry.fallback === undefined ? {} : { fallback: entry.fallback }),
    });
  }
  return fonts;
}

/** 설정을 해석하는 데 필요한 실행 환경 입력입니다. */
export interface ResolveInput {
  /** `--config`로 지정한 설정 파일 경로입니다. 지정한 파일이 없으면 오류가 발생합니다. */
  configPath?: string;
  /** 명령 인자로 지정한 작업 디렉터리입니다. 설정 파일보다 우선합니다. */
  cliRootDir?: string;
  /** 명령 인자 또는 환경변수로 지정한 로케일입니다. 설정 파일보다 우선합니다. */
  cliLocale?: string;
  /** 기본 설정 파일을 찾고 상대 경로를 해석할 때 사용하는 기준 디렉터리입니다. */
  cwd: string;
  /** 암호화 키를 조회할 환경변수입니다. */
  env: Record<string, string | undefined>;
}

/**
 * 설정 파일과 실행 인자를 우선순위대로 합쳐 서버 옵션을 만듭니다.
 *
 * @param input - 실행 인자, 환경변수와 기준 디렉터리입니다.
 * @returns 서버 옵션, 사용한 설정 파일 경로와 PDF 링크 서버 포트를 반환합니다.
 * @throws 설정 파일, 폰트 또는 작업 디렉터리가 잘못되면 {@link SlipMcpConfigError}가 발생합니다.
 */
export async function resolveServerOptions(
  input: ResolveInput,
): Promise<{ options: SlipMcpServerOptions; configPath: string | null; httpPort: number | null }> {
  // 1. 설정 파일 위치를 정합니다. 명시한 파일이 없으면 오류가 발생하지만 기본 위치에는 파일이 없어도 됩니다.
  let configPath: string | null = null;
  let config: SlipMcpConfig = {};
  if (input.configPath !== undefined) {
    configPath = path.resolve(input.cwd, input.configPath);
    config = await readConfigFile(configPath);
  } else {
    const candidate = path.resolve(input.cliRootDir ?? input.cwd, CONFIG_FILE_NAME);
    if (await fileExists(candidate)) {
      configPath = candidate;
      config = await readConfigFile(candidate);
    }
  }
  const baseDir = configPath === null ? input.cwd : path.dirname(configPath);

  // 2. 작업 디렉터리는 CLI 인자, 설정 파일, 기준 디렉터리 순서로 선택합니다.
  const rootDir = path.resolve(
    input.cliRootDir ?? (config.rootDir === undefined ? input.cwd : path.resolve(baseDir, config.rootDir)),
  );
  const info = await stat(rootDir).catch(() => null);
  if (!info?.isDirectory()) {
    throw new SlipMcpConfigError(`Working directory not found: ${rootDir}`);
  }

  // 3. 로케일은 CLI 인자·환경변수, 설정 파일 순서로 선택합니다.
  const locale = input.cliLocale ?? config.locale;

  // 4. 사용자 지정 폰트가 없으면 동봉 폰트를 사용하도록 비워 둡니다.
  const fonts = config.fonts === undefined ? undefined : await loadConfigFonts(config.fonts, baseDir);

  // 5. 암호화 키는 설정 파일이 지정한 환경변수 또는 기본 환경변수에서 읽습니다.
  const keyEnv = config.encryption?.keyEnv ?? DEFAULT_KEY_ENV;
  const previousKeysEnv = config.encryption?.previousKeysEnv ?? DEFAULT_PREVIOUS_KEYS_ENV;
  const key = input.env[keyEnv];
  const previousKeys = input.env[previousKeysEnv]
    ?.split(',')
    .map((entry) => entry.trim())
    .filter((entry) => entry !== '');
  if (config.encryption?.keyEnv !== undefined && key === undefined) {
    throw new SlipMcpConfigError(
      `The config names encryption key variable "${keyEnv}", but it is not set in the environment.`,
    );
  }
  // 비어 있는 키 때문에 평문으로 저장되지 않도록 거부합니다. 설정 오류는 시작할 때 알립니다.
  if (key !== undefined && key.trim() === '') {
    throw new SlipMcpConfigError(
      `Encryption key variable "${keyEnv}" is set but empty. Set a key, or unset the variable to store files unencrypted.`,
    );
  }

  const options: SlipMcpServerOptions = {
    rootDir,
    ...(locale === undefined ? {} : { locale }),
    ...(fonts === undefined ? {} : { fonts }),
    ...(key === undefined
      ? {}
      : { encryption: { key, ...(previousKeys?.length ? { previousKeys } : {}) } }),
  };
  return { options, configPath, httpPort: config.httpPort ?? null };
}

/** 파일이 있는지 확인합니다. */
async function fileExists(filePath: string): Promise<boolean> {
  const info = await stat(filePath).catch(() => null);
  return info?.isFile() ?? false;
}
