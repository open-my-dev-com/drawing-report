...
/** 빌드된 CLI를 자식 프로세스로 실행해 stdout·stderr·종료 코드를 모읍니다. */
...
/** 빌드된 CLI를 서버 모드로 실행하고 stderr의 시작 안내를 기다립니다. */
...
describe('slipkit-mcp 인자 해석', () => {
  ...
  it('모르는 옵션, 옵션 값 누락, 위치 인자 2개 이상은 사용법 오류다', () => {
    ...
    // 인자가 유효하지 않으면 --help가 있어도 사용법 오류입니다.
...
describe('slipkit-mcp 실행', () => {
  it('도움말은 stdout에 본문만 출력하고 서버를 시작하지 않으며 종료 코드 0이다', async () => {
  ...
  it('버전은 stdout에 버전 한 줄만 출력하고 서버를 시작하지 않으며 종료 코드 0이다', async () => {
    const serve = vi.fn(async () => undefined);
    const { io, stdout, stderr } = fakeIo();
...
describe.skipIf(!existsSync(cliPath))('빌드된 dist/cli.js', () => {
  ...
  it('같은 작업 디렉터리의 다른 링크 서버가 포트를 쓰면 사용 가능한 포트에서 새로 실행하고 안내한다', async () => {
    const dir = await makeWorkDir();
