/**
 * 양식과 입력값으로 전표 객체를 생성합니다.
 * core API와 `<slip-form>`이 같은 조립 규칙을 사용합니다.
 */
import type { JsonValue, SlipTemplateFile, SlipVoucherFile } from './schema.js';
import { normalizeNumericParameters } from './normalize.js';

/** JSON 호환 값을 깊은 복사해 입력 객체와의 참조 공유를 제거합니다. */
function clone<T>(value: T): T {
  return JSON.parse(JSON.stringify(value)) as T;
}

/**
 * 양식과 값으로 발행 전(`issued: false`) 전표를 생성합니다.
 *
 * @remarks
 * - 값은 파라미터 키(`key`)를 키로 담습니다. 목록(list) 파라미터는 항목 객체 배열,
 *   이미지 파라미터는 `data:` Base64 문자열입니다(SPEC §7).
 * - `number` 파라미터의 미입력 값, `null`, 빈 문자열은 0으로 변환합니다.
 * - 수식으로 계산되는 값(예: 합계금액)은 `values`에 넣지 않아도 렌더 시 계산됩니다.
 * - 결과는 입력 양식·값과 참조를 공유하지 않는 독립 객체입니다.
 * - 값을 확정하려면 `issued`를 `true`로 설정합니다.
 *
