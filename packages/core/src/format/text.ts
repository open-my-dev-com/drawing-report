/**
 * `.slip` 파일의 텍스트를 JSON으로 해석하기 전에 공통 문자열 처리를 수행합니다.
 */

/** UTF-8 BOM으로 해석하는 문자 (U+FEFF)입니다. */
const BOM = '\uFEFF';

/**
 * 파일 텍스트 맨 앞에 붙은 UTF-8 BOM 하나를 제거합니다.
 *
 * 일부 편집기·운영체제가 UTF-8 파일 앞에 붙이는 BOM은 JSON 문법이 아니므로 파싱 전에 떼어 냅니다.
