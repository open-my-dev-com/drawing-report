/**
 * `.slip` 파일의 JSON Schema를 생성합니다.
 *
 * Zod 스키마에서 draft 2020-12 JSON Schema를 생성해 패키지에 포함합니다.
 * `scripts/generate-json-schema.mjs`로 `packages/core/schemas/`를 갱신합니다.
 * `refine`과 `superRefine`의 교차 필드 검증은 JSON Schema로 표현되지 않으므로
 * 전체 검증에는 `parseSlipFile`을 사용합니다(`docs/SPEC.md` §10).
 */
import { z } from 'zod';
...
/**
 * 현재 `.slip` 스키마 버전의 JSON Schema를 생성합니다.
 *
