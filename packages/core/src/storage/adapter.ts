/**
 * 저장소 어댑터 인터페이스를 정의합니다.
 * 저장, 조회, 삭제, 목록 조회를 정의하며 버전 이력은 선택적으로 지원합니다.
 */
...
export interface SlipListFilter {
  kind?: SlipFileKind;
  /** 제목 등 저장소 구현이 지원하는 필드를 검색할 문자열입니다. */
  query?: string;
}
...
export interface SlipListPage {
  items: SlipListItem[];
  /** 다음 페이지 커서. 없으면 마지막 페이지입니다. */
  nextCursor?: string;
}

/** 파일 본문을 제외한 목록 항목 정보입니다. */
...
/** 호스트 저장소를 연결하는 인터페이스입니다. */
export interface StorageAdapter {
  /**
   * 파일을 저장합니다. 같은 ID가 이미 있으면 덮어씁니다.
   *
   * @param id - 저장 키
  ...
  /**
   * 저장된 파일 목록을 페이지 단위로 반환합니다.
   *
   * @param filter - 종류·검색어 필터(생략하면 전체)
   * @param cursor - 이전 페이지가 반환한 nextCursor입니다. 생략하면 첫 페이지를 조회합니다.
   * @returns 목록 한 페이지
...
export interface VersionedStorageAdapter extends StorageAdapter {
   ...
   *
   * @param id - 저장 키
   * @param version - listVersions가 반환한 버전 식별자
