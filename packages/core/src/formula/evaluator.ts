/**
 * 파싱된 AST를 해석해 수식을 평가합니다. 임의의 JavaScript 코드는 실행하지 않습니다.
 *
 * 참조 경로는 `context.values`에서 조회합니다. 경로 중간에 배열이 있으면 남은 경로를
 * 각 원소에 적용해 배열로 반환합니다. `IF`, `AND`, `OR`는 단락 평가합니다.
 */
...
/** 값 참조를 순회할 수 있는 최대 깊이입니다. */
const MAX_VALUE_DEPTH = 256;

function guardDepth(depth: number): void {
  // 참조를 해석할 때 발생하는 오류는 모두 데이터의 구조나 값에서 비롯됩니다.
...
function resolvePath(value: unknown, path: string[], index: number, depth = 0): FormulaValue {
  ...
  if (typeof value === 'object') {
    // 객체가 직접 가진 키만 읽어 프로토타입의 메서드가 값으로 새어 나오지 않게 합니다.
...
function applyBinary(
  ...
  switch (operator) {
    ...
    case '<>': return !equals(a, b);
    default: {
      // 순서 비교는 두 피연산자가 모두 숫자이거나 모두 문자열일 때만 허용합니다.
      ...
      // 한쪽이라도 데이터에서 왔으면 값이 달라질 때 종류가 맞을 수 있습니다.
...
/**
 * 평가한 값과 값의 출처를 함께 나타냅니다.
 *
 * @remarks
 * 출처는 실제로 평가한 자리에서만 정합니다. 단락 평가로 건너뛴 `IF` 분기의 참조는 고른 값의
 * 출처가 아니므로 반영하지 않습니다. 평가기 안에서만 쓰고 공개 API로 내보내지 않습니다.
 */
interface Evaluated {
  /** 평가 결과 값입니다. */
  value: FormulaValue;
  /** 이 값이 참조한 데이터에서 왔는지를 나타냅니다. */
  fromData: boolean;
}
...
// 진단 중에만 값이 없어 실패한 자리를 빈 값으로 잇고 계속 평가합니다.
...
function evaluateAst(ast: FormulaAst, context: FormulaContext): Evaluated {
  try {
    ...
    // 진단 중에는 데이터에 의존한 평가 실패를 기록하고 빈 값으로 계산을 이어 갑니다.
    // 이후 연산에서 발생하는 오류를 함께 수집하기 위한 처리입니다.
 ...
 *
 * @remarks
 * 진단 중에 데이터 값 때문에 실패하면, 데이터에서 온 인자를 빈 값으로 바꿔 한 번 더 호출합니다.
 * 그래야 `FORMAT_NUMBER($(amount), 21)`처럼 잘못된 상수 인자가 값 오류 뒤에 가려지지 않습니다.
 */
function callBuiltin(
  ...
  try {
    ...
    try {
      ...
      // 빈 값으로 바꾼 자리에서 발생한 오류는 별도의 오류로 보고하지 않습니다.
...
function evaluateNode(ast: FormulaAst, context: FormulaContext): Evaluated {
  switch (ast.type) {
    ...
    case 'reference': {
      ...
      // 예약 참조(@item 등)는 페이지 계획이 전달한 reserved에서만 조회합니다. `$(@item)`처럼
      // 키로 적은 이름은 예약 참조가 아니라 값 키이므로 파서가 남긴 표시로만 구분합니다.
    ...
    case 'call': {
      // 단락 평가가 필요한 함수는 인수를 개별적으로 평가합니다.
      if (ast.name === 'IF') {
        ...
        // 고른 분기만 평가하므로 건너뛴 분기의 출처는 결과에 섞이지 않습니다.
      ...
      if (ast.name === 'AND' || ast.name === 'OR') {
        ...
        // 단락 평가로 건너뛴 인수는 결과의 출처에 넣지 않습니다.
        let seenData = false;
      ...
      // 파서에 등록됐지만 평가기에 구현되지 않은 함수를 확인합니다.
...
export function evaluateFormula(source: string | FormulaAst, context: FormulaContext): FormulaValue {
  return withFormulaLocale(context.locale, () => {
    ...
    // 평가는 첫 오류에서 멈춥니다. 진단 중에 불려도 오류를 이어 붙이지 않습니다.
...
/** 현재 평가에서 발견한 오류를 분류한 결과입니다. */
export interface FormulaDiagnosis {
  /**
   * 계산 결과.
   *
   * @remarks
   * `formulaError`나 `dataError`가 있으면 진단 과정에서 나온 값이므로 결과로 사용하지 않습니다.
   */
  value: FormulaValue;
  /** 현재 평가에서 수식 구성에 관한 오류로 분류된 평가 오류입니다. */
  formulaError?: FormulaEvalError;
  /** 현재 평가에서 데이터에 의존한다고 분류된 평가 오류입니다. */
  dataError?: FormulaEvalError;
}

/**
 * 현재 값으로 수식을 평가해 발견한 오류를 분류합니다.
 *
 * @remarks
 * 데이터 관련 실패 지점에는 빈 값을 대입해 나머지 식을 평가하고, 현재 평가에서 발견한
 * 오류를 `dataError`와 `formulaError`로 나눠 반환합니다. 이 분류는 저장 허용 여부를 뜻하지
 * 않습니다.
 *
 ...
 * found.dataError !== undefined; // true - 예약 범위가 없어 현재 평가가 실패했습니다.
 * found.formulaError === undefined; // true - 수식 구성 오류로 분류된 것은 없습니다.
 * ```
 */
