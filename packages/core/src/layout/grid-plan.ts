/**
 * 반복 그리드 하나를 출력 페이지 조각으로 나누어 배치 계획을 만듭니다.
 *
 * 계획은 순수 데이터 계산이며 렌더링 엔진에 의존하지 않습니다. 같은 입력은 항상
 * 같은 결과를 반환합니다. 항목 구간은 원자 단위로 취급해 페이지 사이에서 나누지 않습니다.
 */
...
/** 출력 페이지 조각 안에 배치된 행 구간 인스턴스입니다. */
export interface PlannedBand {
  /** 원본 행 구간입니다. */
  band: GridBand;
  /** 조각 안에서 이 인스턴스가 시작하는 출력 행 인덱스입니다. */
  rowStart: number;
  /**
   * 항목·그룹 구간이 참조하는 실제 항목 인덱스.
   * 항목 구간은 자신의 항목, 그룹 구간은 그룹의 첫(시작)·마지막(종료) 항목입니다.
   * 빈 항목 인스턴스에는 없습니다.
   */
  itemIndex?: number;
  /** 값 없이 공간만 채우는 빈 항목 인스턴스인지 나타냅니다. */
  emptyItem?: boolean;
  /** 그룹 설정이 있는 항목·그룹 구간의 그룹 인덱스입니다. */
  groupIndex?: number;
}

/** 한 출력 페이지에 배치되는 그리드 조각입니다. */
export interface GridFragment {
  /** 원본 양식 페이지 안의 출력 페이지 번호입니다. 0부터 시작합니다. */
  outputPage: number;
  /** 조각이 시작하는 용지 절대 Y 좌표(mm)입니다. */
  y: number;
  /** 조각의 전체 높이(mm)입니다. */
  height: number;
  /** 조각의 출력 행 높이 목록입니다. */
  rowHeights: number[];
  /** 배치된 행 구간 인스턴스를 세로 순서로 담습니다. */
  bands: PlannedBand[];
  /** 이 페이지에 배치된 실제 항목 인덱스(`@page`)입니다. */
  pageItems: number[];
  /**
   * 이 페이지 이전까지 배치된 실제 항목 수 (`@carried`).
   * 항목은 인덱스 순서로 배치되므로 이월 항목은 실제 항목 배열의 앞 `carriedCount`개입니다.
   */
  carriedCount: number;
}

/** 그리드 하나의 페이지 계획 결과입니다. */
export interface GridPlan {
  gridId: string;
  /** 출력 페이지별 조각입니다. `outputPage` 오름차순으로 정렬됩니다. */
  fragments: GridFragment[];
  /** `maxItems`를 적용한 실제 항목 수입니다. */
  itemCount: number;
  /** 실제 항목 인덱스에 대응하는 그룹 인덱스입니다. 그룹 설정이 없으면 빈 배열입니다. */
  groupOf: number[];
  /** 그룹별 실제 항목 인덱스 목록입니다. */
  groups: number[][];
}

/** 그리드 흐름을 배치할 세로 공간입니다. */
export interface GridFlow {
  /** 첫 조각이 시작하는 출력 페이지 번호입니다. */
  firstPage: number;
  /** 첫 조각의 시작 Y 좌표입니다. 그리드의 `position.y` 또는 `after` 배치 결과를 사용합니다. */
  firstTop: number;
  /** 이어지는 조각이 시작하는 Y 좌표인 흐름 영역 상단입니다. */
  top: number;
  /** 조각이 넘을 수 없는 Y 좌표인 흐름 영역 하단입니다. */
  bottom: number;
  /**
   * `after` 배치로 시작이 줄어든 흐름인지.
   * 참이면 실제 항목이 없어도 첫 페이지에 아무것도 들어가지 않을 때 빈 조각 없이 다음
   * 페이지에서 시작합니다. 실제 항목이 하나 이상인 그리드는 이 값과 무관하게 같은 규칙으로
   * 시작 위치를 옮깁니다({@link planGrid} 참고).
   */
  allowStartShift?: boolean;
}

/** 구간에 포함된 템플릿 행 높이의 합을 구합니다. */
...
/** 최종 페이지 여부를 포함해 표시 페이지 필터가 해당 페이지에 적용되는지 확인합니다. */
...
/** 두 항목이 groupBy 기준으로 같은 그룹인지 비교합니다. */
...
/** 계획 단계에서 페이지에 배치할 원자 단위 블록입니다. */
interface PlanBlock {
  bands: PlannedBand[];
  height: number;
  /** 블록에 포함된 실제 항목 인덱스 (빈 항목은 제외)입니다. */
  itemIndex?: number;
}

/** placement 종류별 구간 목록을 만듭니다. */
...
/**
 * 반복 그리드 하나의 페이지 계획을 만듭니다.
 *
 * 첫 조각의 시작 위치가 흐름 영역 상단보다 낮아 항목 인스턴스(실제 항목·빈 항목)를 하나도
 * 배치하지 못하면, 첫 페이지에 조각을 남기지 않고 다음 출력 페이지의 흐름 영역 상단에서
 * 다시 시작합니다(시작 이동). 시작 이동은 첫 조각에서 한 번만 일어나며, 실제 항목이 하나 이상인
 * 그리드와 `after` 배치 흐름({@link GridFlow.allowStartShift})에 적용됩니다. 실제 항목이 없는
 * 절대 배치 그리드는 빈 항목을 `position.y`부터 그대로 배치합니다. 옮긴 뒤에도 빈 페이지의
 * 흐름 영역 전체에 머리·꼬리 구간, 항목 블록 또는 고정 페이지 묶음이 들어가지 않으면
 * 빈 조각을 만들지 않고 바로 오류를 반환합니다.
 *
...
export function planGrid(
  ...
  // 항목 인스턴스 수: 자동 확장은 minItems까지, 고정 페이지는 페이지 단위로 빈 항목을 채웁니다.
  ...
  /** 항목 인스턴스 하나를 그룹 시작·종료 규칙이 반영된 배치 블록으로 만듭니다. */
  ...
  let index = 0;
  let fragmentNo = 0;
  /** 페이지를 넘긴 그룹의 시작 구간을 다음 페이지 머리에 다시 표시하기 위한 목록입니다. */
  let pendingGroupRepeat: PlannedBand[] = [];

  /** 페이지 시작 구간(첫 페이지의 before-data 포함)의 인스턴스를 만듭니다. */
  ...
  /** 조각을 마무리해 rowStart·rowHeights를 계산하고 목록에 추가합니다. */
  ...
  /**
   * 남은 인스턴스 전체가 이 페이지에 들어가는지 검사합니다.
   * 들어가면 이 페이지가 마지막 출력 페이지입니다.
   */
  const remainderFits = (available: number, isFirst: boolean): boolean => {
    let height = heightOf(headBands(isFirst, true));
    // 구간 높이는 모두 양수라 한 번 넘치면 남은 항목을 더 세지 않아도 결과가 같습니다.
    // 항목이 많은 문서에서 페이지마다 남은 항목 전체를 훑지 않도록 여기서 멈춥니다.
  ...
  // 실제 항목이 있는 그리드는 배치 방식과 무관하게, after 배치 흐름은 항목이 없어도 옮깁니다.
  const canShiftStart = flow.allowStartShift === true || real.length > 0;

  /**
   * 항목 인스턴스를 하나도 배치하지 못한 첫 페이지를 빈 조각 없이 건너뛰고 다음 출력
   * 페이지의 흐름 영역 상단에서 시작합니다.
   * 첫 조각을 만들기 전이고 시작 위치가 흐름 영역 상단보다 낮을 때만 옮길 수 있습니다.
   * 아직 아무 인스턴스도 배치하지 않았을 때만 사용합니다.
   */
  ...
  for (;;) {
    ...
    // 흐름 영역 상단에서 시작하는 페이지는 빈 페이지 전체를 씁니다. 여기서 넘치면 다음
    // 페이지도 같은 구성이므로 시작을 옮기거나 페이지를 넘겨도 해결되지 않습니다.
    ...
    // 이월된 그룹 시작 구간은 페이지 머리 구간 바로 뒤에 다시 표시합니다.
    ...
    if (pagination.mode === 'fixed') {
      // 고정 페이지: 정확히 itemsPerPage개의 항목 영역을 배치합니다.
    ...
    while (index < instanceCount) {
      ...
      if (used + block.height + tailReserve > available + 0.001) {
        ...
        // 이월된 그룹 머리를 포함해 더 진행할 수 없으므로 빈 조각 없이 즉시 오류를 반환합니다.
    ...
    if (finishedItems && !pageBroken) {
      ...
      // 최종 전용 구간이나 합계가 이 페이지에 들어가지 않습니다.
      // 비최종 구간으로 마감하고 새 출력 페이지에 최종 구간을 배치합니다(§6.2).
      ...
      // 최종 페이지 전용 구간이 없으면 추가 페이지를 만들지 않습니다.
    ...
    // 페이지가 넘어갔으므로 이 페이지를 마감하고 다음 페이지에서 계속합니다.
    bands.push(...tailBands(isFirst, false, false));
    commitFragment(outputPage, y, bands);
    fragmentNo++;

    // 페이지를 넘긴 그룹의 시작 구간을 다음 페이지에 다시 표시합니다.
    queueGroupRepeat();
  }
