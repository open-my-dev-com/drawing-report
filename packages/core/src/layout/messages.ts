...
const KO: LayoutMessages = {
  ...
  fixedPageTooTall: (what, itemsPerPage, space) =>
    `${what}에 페이지당 ${itemsPerPage}개 항목을 배치하면 ${KO_SPACE[space]}에 들어가지 않습니다`,
  groupTooTall: (what) =>
    `${what}의 항목 블록이 ${KO_SPACE['empty-page']}에 들어가지 않습니다. 이 블록에는 페이지를 넘길 때 다시 표시되는 그룹 시작 구간이 포함됩니다`,
  bandsTooTall: (what, space) => `${what}의 고정 행 구간이 ${KO_SPACE[space]}에 들어가지 않습니다`,
  flowOverlap: (a, b) => `두 출력 영역이 겹칩니다: ${a}, ${b}`,
  outputPagesExceeded: (max) => `페이지 계획이 출력 페이지 상한(${max})을 초과합니다`,
  elementTooTall: (what) => `${what}의 높이가 페이지 흐름 영역보다 큽니다`,
  planNotConverged: () => '이어서 배치할 때 출력 페이지 수를 확정할 수 없습니다',
};
