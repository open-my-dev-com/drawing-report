 ...
 * @throws SlipLayoutError 흐름 영역 초과, 출력 영역 겹침 또는 출력 페이지 상한 초과 시
 */
export function planSourcePage(
  ...
  for (const element of ordered) {
    ...
    // 절대 배치 요소의 출력 끝은 원본 위치 기준이다 (첫 출력 페이지).
    flowEnd.set(element.id, { outputPage: 0, y: element.position.y + bounds.height });
  }
  ...
  const gridRects: { id: string; name: string; root: string; page: number; x: number; y: number; w: number; h: number }[] = [];
  for (const element of page.elements) {
    if (element.type !== 'grid' || element.repeat === undefined) continue;
  ...
  for (let a = 0; a < gridRects.length; a++) {
    for (let b = a + 1; b < gridRects.length; b++) {
      const ra = gridRects[a]!;
      const rb = gridRects[b]!;
      if (ra.root === rb.root || ra.page !== rb.page) continue;
      const overlap = ra.x < rb.x + rb.w && rb.x < ra.x + ra.w && ra.y < rb.y + rb.h && rb.y < ra.y + ra.h;
      if (overlap) {
        throw new SlipLayoutError(
          lm(locale).flowOverlap(
            lm(locale).subjectGrid(ra.name, ra.id),
            lm(locale).subjectGrid(rb.name, rb.id),
          ),
          { elementId: rb.id },
        );
      }
