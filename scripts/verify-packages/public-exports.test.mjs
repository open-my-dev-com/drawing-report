// 공개 export 허용 목록(`fixtures/public-exports.json`)이 코드와 어긋나지 않는지 `node --test`로 확인합니다.
// tarball 없이 저장소의 빌드 산출물(packages/*/dist)만 읽습니다. `pnpm verify`는 build 뒤에 이 시험을 돌립니다.
...
/** `$comment`를 제외하고 허용 목록의 패키지 항목만 반환합니다. */
function packages() {
...
/**
 * d.ts의 `export { ... }` 문을 모두 읽어 이름을 모읍니다. `type` 표시가 있으면 타입으로, 없으면 값으로 분류하되
 * 다른 패키지에서 다시 내보낸 타입은 `type` 표시 없이 나올 수 있으므로 값 분류는 "타입일 수도 있음"으로 판단합니다.
 */
function readDeclaredExports(file) {
  const text = readFileSync(file, 'utf8');
  assert.equal(/^export \*/m.test(text), false, `${file}: export *로는 이름을 확정할 수 없습니다.`);
  const typed = new Set();
  ...
  for (const match of text.matchAll(/^export\s+(type\s+)?\{([^}]*)\}/gm)) {
    ...
    for (const entry of match[2].split(',').map((item) => item.trim()).filter(Boolean)) {
      ...
      assert.ok(parsed, `${file}: export 항목을 해석할 수 없습니다: ${entry}`);
...
describe('공개 export 허용 목록', () => {
    ...
    for (const [subpath, expected] of Object.entries(subpaths)) {
      ...
      it(`${pkg} ${subpath}: 런타임 export가 허용 목록과 같다`, async () => {
        ...
        for (const name of expected.removed?.runtime ?? []) {
          assert.equal(name in module, false, `공개 API에서 제외한 이름이 여전히 export됩니다: ${name}`);
        }
      });
      ...
      it(`${pkg} ${subpath}: d.ts가 선언하는 이름이 런타임·타입 허용 목록의 합과 같다`, () => {
        ...
        assert.ok(result.ok, `missing: ${result.missing.join(', ') || '-'} / extra: ${result.extra.join(', ') || '-'}`);
        // `type` 표시가 붙은 이름은 런타임 목록에 있을 수 없습니다.
        const typedRuntime = [...declared.typed].filter((name) => expected.runtime.includes(name));
        assert.deepEqual(typedRuntime, [], `타입으로만 선언됐지만 런타임 목록에도 있습니다: ${typedRuntime.join(', ')}`);
        for (const name of [...(expected.removed?.runtime ?? []), ...(expected.removed?.types ?? [])]) {
          assert.equal(all.includes(name), false, `공개 API에서 제외한 이름이 d.ts에 여전히 선언되어 있습니다: ${name}`);
        }
      });
    }
  }
  ...
  it('public-types/index.ts가 허용 목록의 모든 이름과 뺀 이름을 다룬다', () => {
    ...
    assert.deepEqual(absent, [], `public-types/index.ts에 없는 이름입니다: ${absent.join(', ')}`);
  });
});
