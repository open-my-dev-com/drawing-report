/**
 * `bench:mcp-list`에서 사용하는 `.slip` 시험 자료를 만듭니다.
 *
 * 만드는 파일은 모두 `parseSlipFile`을 통과하는 유효한 `.slip`입니다. 목록 조회가 파싱까지 하므로
 * 손상 파일을 섞으면 측정 대상이 달라집니다. 파일은 `shard-000/slip-00000.slip`처럼 하위 디렉터리로
 * 나눠 두어 재귀 탐색 비용도 함께 재고, 이름을 0으로 채워 정렬 순서가 색인 순서와 같게 합니다.
 *
 * 색인 규칙은 모든 시나리오에서 같습니다.
 * - `index % 5 === 0`이면 전표, 나머지는 양식 — `kind` 필터가 고르는 비율이 8:2가 됩니다.
 * - `index % 10 === 3`이면 제목에 검색 표시 문자열 `alpha`를 붙입니다. `query` 필터는 약 10%를 선택합니다.
 * - 색인 0은 항상 제목 끝에 `r00`처럼 두 자리 수정 번호를 붙입니다. 외부 수정 단계에서는 이 자리만
 *   바꾸므로 파일 크기는 그대로 두고 내용과 mtime만 달라집니다.
 *
 * 크기 맞추기
 * - `targetBytes`가 null이면 최소 유효 양식(텍스트 요소 하나)을 그대로 씁니다.
 * - 값을 주면 더미 텍스트 요소를 늘려 그 바이트에 맞춥니다. 요소 수는 종류마다 한 번만 계산하고,
 *   파일마다 달라지는 제목 길이는 마지막 더미 요소의 글자 수로 흡수해 정확히 같은 바이트로 만듭니다.
 */
import { mkdirSync, readdirSync, writeFileSync } from 'node:fs';
import path from 'node:path';

/** 하위 디렉터리 하나에 넣는 파일 수입니다. */
const SHARD_SIZE = 100;

/** 제목에 붙이는 검색 표시 문자열입니다. `query` 필터가 이 문구를 찾습니다. */
export const QUERY_MARK = 'alpha';

/** 외부 수정 단계에서 고치는 파일의 색인입니다. */
export const TOUCH_INDEX = 0;

/** 삭제·추가 단계에서 지우는 파일의 색인입니다. */
export const CHURN_INDEX = 1;

/** 삭제·추가 단계에서 만드는 파일의 저장 키입니다. 지운 파일 자리에 정렬되어 첫 페이지에 포함됩니다. */
export const ADDED_ID = 'shard-000/slip-00001-added.slip';

/** 더미 텍스트 요소 하나의 기본 글자 수입니다. */
const PAD_CHARS = 48;

/**
 * 색인에 해당하는 상대 경로를 만듭니다.
 *
...
/**
 * 색인의 제목에 검색 표시 문자열을 붙일지 정합니다.
 *
 * @param index - 파일 색인
 * @returns 표시 문자열을 붙이면 true
 */
...
/**
 * 파일 수에 대한 기대 항목 수를 계산합니다. 목록 결과 검증에 씁니다.
 *
 * @param count - 시험 자료 파일 수
 * @returns 전체·양식·검색 표시 항목 수
 */
 ...
 *
 * @param index - 파일 색인
 * @param revision - 두 자리 수정 번호입니다. 생략하면 번호를 붙이지 않습니다.
 * @returns 제목 문자열
 */
...
/**
 * 파일 본문의 마지막 요소를 찾습니다. 양식과 전표의 본문 위치가 달라 한 곳에서 처리합니다.
 *
...
/**
 * 시험 자료 생성기를 만듭니다. 같은 규칙으로 파일 객체·직렬화 문자열을 얻을 수 있어,
 * 처음 만들 때와 외부 수정 단계가 다시 쓸 때 같은 내용을 재현합니다.
 *
...
export function createGenerator({ serializeSlipFile, schemaVersion, targetBytes = null }) {
  /** 종류별 더미 요소 수입니다. 종류마다 한 번만 계산합니다. */
  const padCounts = new Map();

  const sizeOf = (file) => Buffer.byteLength(serializeSlipFile(file), 'utf8');

  // 제목이 가장 긴 파일(검색 표시 문자열과 수정 번호가 모두 있는 경우)을 기준으로 요소 수를 정해야
  // 어떤 파일에서도 목표 바이트를 넘지 않고 마지막 요소로 차이를 메울 수 있습니다.
  ...
  /**
   * 마지막 더미 요소의 글자 수로 파일 크기를 목표 바이트에 맞춥니다.
   *
   * @param file - `.slip` 파일 객체
   ...
   *
   * @param index - 파일 색인
   * @param revision - 두 자리 수정 번호(색인 0에만 사용)
   * @returns `.slip` 파일 객체
   */
  function fileFor(index, revision) {
    ...
    // 제목 길이가 파일마다 달라지므로 마지막 더미 요소의 글자 수로 차이를 흡수합니다.
  ...
  /**
   * 삭제·추가 단계가 새로 만드는 파일의 내용을 만듭니다.
   *
   * @returns 검색 표시 문자열이 없는 양식의 `.slip` JSON 문자열
   */
  function addedText() {
...
/**
 * 디렉터리가 없으면 만듭니다. 같은 하위 디렉터리를 여러 번 만들지 않도록 기억합니다.
 *
 * @param dir - 만들 디렉터리
...
/**
 * 평문 시험 자료를 만듭니다. 동기 API로 한 번에 써서 생성 시간이 측정에 섞이지 않게 합니다.
 *
...
/**
 * 암호화한 시험 자료를 만듭니다. 암호화는 비동기이므로 묶음으로 나눠 처리하고, 쓰기는 동기로 합니다.
 *
...
/**
 * 디렉터리 항목 수(파일과 하위 디렉터리)를 셉니다. 측정 심볼이 없어도 이 값으로 탐색 규모를 기록합니다.
 *
 * @param dir - 기준 디렉터리
 * @returns 항목 수
 */
