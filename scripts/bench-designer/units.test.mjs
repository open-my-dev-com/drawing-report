// benchmark가 문자 수를 문자 수로 보고하는지 고정하는 시험 — `node --test`로 실행한다.
//
// 되돌리기 스냅샷 길이와 양식 직렬화 길이는 UTF-16 코드 단위 수다. 표 머리말·행과 JSON 지표 단위,
// 기준선이 한 곳이라도 바이트로 표기되면 실제로 재는 것과 어긋나므로 원문을 직접 읽어 확인한다.
...
describe('bench-designer 표기', () => {
  ...
  it('문자 수 지표를 count 단위로 낸다', () => {
    for (const key of ['snapshotChars', 'templateChars']) {
      ...
      assert.ok(line !== undefined, `${key} 지표 선언이 없다`);
...
describe('성능 기준선', () => {
  ...
  it('문자 수 지표의 단위와 이름이 문자 수로 적혀 있다', () => {
    ...
    assert.ok(checked > 0, '문자 수 지표를 하나도 찾지 못했다');
  });

  it('바이트 단위 지표는 실제 바이트를 재는 것만 남는다', () => {
    for (const name of files) {
      for (const metric of JSON.parse(read(path.join(dir, name))).metrics) {
