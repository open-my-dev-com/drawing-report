// 폰트 예산 정적 분석 모듈의 시험입니다. `node --test`로 실행하며 임시 디렉터리에 합성 패키지를 만들어 분류·정적 의존 파일·예산 비교를
// 확인하고, 마지막에 실제 `packages/elements/dist`(있을 때만)에 대해 `pnpm pack` 없이 예산이 통과하는지 확인합니다.
...
/** 시험에서 만든 임시 디렉터리입니다. 시험이 끝나면 모두 삭제합니다. */
const tempDirs = [];
after(() => {
  for (const dir of tempDirs) rmSync(dir, { recursive: true, force: true });
});

/** 합성 dist의 기본 파일 내용입니다. */
const DEFAULT_FILES = {
  ...
  // 루트의 정적 의존 파일에 들지 않는 재수출 진입점은 청크 파일에도 들어가지 않아야 합니다.
...
/**
 * 합성 패키지를 임시 디렉터리에 만듭니다.
 *
 * @param {Record<string, string | null>} [overrides] - 덮어쓸 파일 내용. `null`이면 그 파일을 만들지 않습니다.
...
/**
 * 파일들의 raw·gzip 합을 시험 쪽에서 독립적으로 계산합니다.
 *
...
/**
 * 비동기 호출이 지정한 종류의 `FontBudgetError`를 던지는지 확인합니다.
 *
...
async function rejectsWithKind(fn, kind) {
  await assert.rejects(fn, (error) => {
    assert.ok(error instanceof FontBudgetError, `FontBudgetError가 아닙니다: ${error}`);
    assert.equal(error.kind, kind);
    return true;
  });
}

describe('정적 import 의존 파일 집합', () => {
...
describe('폰트 청크 분류와 예산 비교 (합성 패키지)', () => {
  it('폰트 exports 대상과 export 이름으로 종류를 정한다', async () => {
    ...
    // 청크 파일은 청크 진입점의 정적 의존 파일에서 루트의 정적 의존 파일을 뺀 결과입니다.
  ...
  it('디코딩한 데이터의 바이트 수를 폰트 이름별로 반환한다', async () => {
    const dir = makePackage();
    const chunks = await classifyFontChunks(dir);
  ...
  it('상한을 넘으면 ok가 false이고 실패 행이 정확히 그 항목이다', async () => {
    ...
    assert.deepEqual(result.rows.filter((row) => !row.ok).map((row) => row.item), ['루트 정적 의존 파일 원본', 'Noto Sans JP 디코딩 데이터']);
    const overRow = result.rows.find((row) => row.item === '루트 정적 의존 파일 원본');
...
describe('실제 Elements dist', () => {
  it('두 폰트 청크를 찾고 예산을 통과한다 (pnpm pack 생략)', async (t) => {
    if (!existsSync(path.join(ELEMENTS_DIR, 'dist', 'index.js'))) {
      t.skip('packages/elements/dist가 없습니다. 먼저 pnpm -r build를 실행하세요.');
      return;
    }
