/**
 * Elements 빌드 산출물(dist)의 크기를 재고 동봉 폰트 예산과 비교하는 정적 분석 모듈입니다.
 *
 * - 루트 진입점(`dist/index.js`)에서 상대 경로 정적 import/export-from만 따라간 파일 묶음을
 *   정적 import 의존 파일 집합으로 측정합니다. 패키지 이름 지정자(`lit`, `@omdc-slipkit/core`)와 동적 `import()`는
 *   따라가지 않습니다. 루트를 불러올 때 실제로 읽히는 파일만 세기 위한 기준입니다.
 * - 폰트 청크는 파일 이름을 하드코딩하지 않고 분류합니다. `package.json`의 `./fonts/*` exports가
 *   가리키는 두 파일이 루트의 정적 의존 파일 안에서 가져오는 대상에 포함되고, 각 모듈이 대응하는
 *   `PRETENDARD_FONTS` 또는 `NOTO_SANS_JP_FONTS`를 export할 때만 성공합니다.
 * - gzip 바이트는 `node:zlib` `gzipSync` 기본 레벨로 파일마다 압축한 크기의 합입니다.
 * - tarball 크기는 `pnpm pack` 결과 파일 크기, 압축 해제 크기는 그 tarball의 tar 항목 크기 합입니다
 *   (외부 `tar` 명령 없이 Node에서 gunzip 후 헤더를 읽습니다).
 */
...
/** 정적 import — `import x from "./a.js"`, `import { a,\n b } from "./a.js"`, `import "./a.js"` (여러 줄 허용)입니다. */
const STATIC_IMPORT_RE = /\bimport\s+(?:[\w$*{}\s,]+?\s+from\s+)?["']([^"'\n]+)["']/g;
/** 정적 재수출 — `export { a } from "./a.js"`, `export * from "./a.js"`, `export * as ns from "./a.js"`입니다. */
const STATIC_EXPORT_RE = /\bexport\s+(?:\*(?:\s+as\s+[\w$]+)?|\{[^}]*\})\s+from\s+["']([^"'\n]+)["']/g;
/** 동적 import — `import("./a.js")`·`import('./a.js')`입니다. */
const DYNAMIC_IMPORT_RE = /\bimport\s*\(\s*["']([^"'\n]+)["']\s*\)/g;

/**
 * 상대 경로 지정자인지 확인합니다. bare 지정자와 URL은 따라가지 않습니다.
 *
...
/**
 * 상대 지정자를 절대 파일 경로로 바꿉니다. 확장자가 없으면 `.js`·`/index.js`를 차례로 시도합니다.
 *
 * @param {string} fromFile - 지정자가 적힌 파일
 * @param {string} specifier - 상대 지정자
 * @returns {string} 절대 경로입니다. 파일이 없어도 첫 후보를 반환합니다.
 */
...
/**
 * 한 파일의 상대 경로 정적 import·export-from 대상을 절대 경로로 모읍니다.
 *
...
/**
 * 진입점에서 상대 경로의 정적 import와 export-from만 재귀적으로 따라가 파일 집합을 만듭니다.
 *
...
function closureFiles(entryFile) {
  ...
  if (!existsSync(entry)) throw new Error(`진입점이 없습니다: ${entry}`);
  const seen = new Set([entry]);
  const queue = [entry];
  while (queue.length > 0) {
    const file = queue.shift();
    for (const target of staticTargets(file)) {
      if (!existsSync(target)) throw new Error(`${file}이 가리키는 파일이 없습니다: ${target}`);
...
/**
 * 파일의 원본 바이트 합과 파일별 gzip(기본 레벨) 바이트 합을 계산합니다.
 *
...
/**
 * 상대 경로의 정적 import와 export-from만 따라가 파일 목록과 원본·gzip 바이트 합을 계산합니다.
 * 파일 목록은 진입점을 포함한 절대 경로이며 정렬된 상태입니다.
 *
 * @param {string} entryFile - 진입점 파일 경로
 * @returns {{ files: string[], raw: number, gzip: number }} 정적 의존 파일과 크기 합
 */
...
/**
 * 주어진 파일에 나타나는 상대 경로의 동적 import 대상을 찾습니다.
 *
...
/**
 * `exports[subpath].import` 또는 문자열 exports가 가리키는 파일의 절대 경로를 찾습니다.
 *
 * @param {string} packageDir - 패키지 루트
...
/**
 * 루트 진입점 경로 — `exports["."].import` → `main` → `dist/index.js` 순으로 찾습니다.
 *
 * @param {string} packageDir - 패키지 루트
 * @param {any} pkg - 파싱된 package.json
...
/**
 * 모듈에서 `{ name, data }` 항목으로 이루어진 폰트 배열 export를 찾습니다.
 *
 ...
 * dist 안 폰트 청크를 분류합니다.
 *
 * `package.json` exports(`./fonts/pretendard`·`./fonts/noto-sans-jp`)가 가리키는 두 파일이 루트 진입점의 정적 의존 파일에서
 * 동적 import 대상에 포함되고, 각 모듈이 대응하는 `PRETENDARD_FONTS`/`NOTO_SANS_JP_FONTS`만 export할 때
 * 성공합니다. 폰트와 무관한 동적 import 대상은 분류에서 제외합니다. 청크의 `files`에는 각 진입점의
 * 정적 의존 파일 중 루트 진입점에서도 사용하는 파일을 제외한 결과를 담습니다.
 *
 * @param {string} packageDir - 패키지 루트 (`package.json`·`dist`를 읽습니다)
...
export async function classifyFontChunks(packageDir) {
  ...
  if (missing.length > 0) {
    throw new FontBudgetError('missing-chunk', `동적 import 대상 파일이 없습니다: ${listRelative(dir, missing)}`);
  }
  const exported = {};
  for (const [key, subpath] of Object.entries(CHUNK_SUBPATHS)) {
    const target = exportTarget(dir, pkg, subpath);
    if (target === undefined) throw new FontBudgetError('classification-failure', `package.json exports에 ${subpath} 항목이 없습니다.`);
    exported[key] = target;
  }
  ...
  if (missingExportFiles.length > 0) {
    throw new FontBudgetError('missing-chunk', `폰트 exports 대상 파일이 없습니다: ${listRelative(dir, missingExportFiles)}`);
  }
  ...
  if (unreferenced.length > 0) {
    throw new FontBudgetError(
      ...
      `폰트 exports 대상이 루트 진입점의 동적 import 목록에 없습니다: ${listRelative(dir, unreferenced)}`,
    );
  }
  ...
  for (const [key, entry] of Object.entries(exported)) {
    ...
    if (kinds.length !== 1 || kinds[0] !== key) {
      throw new FontBudgetError(
        'classification-failure',
        `${CHUNK_SUBPATHS[key]} 대상 ${path.relative(dir, entry)}의 폰트 배열 export가 올바르지 않습니다(${kinds.join(', ') || '없음'}).`,
      );
    }
  ...
  // 결과 순서는 파일 이름과 관계없이 청크 키 순서(pretendard, notoSansJp)로 고정합니다.
  const ordered = {};
  for (const key of Object.keys(CHUNK_EXPORTS)) {
    if (classified[key] === undefined) throw new FontBudgetError('classification-failure', `${CHUNK_LABELS[key]}를 찾지 못했습니다.`);
...
/**
 * 청크 모듈을 import해 export된 폰트 배열 항목의 `data.length`를 폰트 이름별로 모읍니다.
 *
 * @param {{ entry: string, exportName?: string }} chunk - `classifyFontChunks`가 반환한 청크
...
export async function decodedFontBytes(chunk) {
  ...
  if (fonts === undefined) throw new FontBudgetError('classification-failure', `${chunk.entry}에 폰트 배열 export(${names.join(', ')})가 없습니다.`);
  return Object.fromEntries(fonts.map((font) => [font.name, font.data.length]));
}

/**
 * tar 아카이브 바이트에서 일반 파일 항목의 크기 필드를 합산합니다(512바이트 헤더, 8진수 size, 데이터 블록 512 정렬).
 *
...
function tarUnpackedBytes(tar) {
  ...
  while (offset + BLOCK <= tar.byteLength) {
    ...
    let size;
    if (sizeField[0] & 0x80) {
      // base-256 표기(GNU 확장)는 아주 큰 항목에만 사용됩니다.
      size = 0;
...
/**
 * `pnpm pack --pack-destination <임시 디렉터리>`로 tarball을 만들어 크기와 unpacked 크기를 측정합니다. 끝나면 임시 디렉터리를 지웁니다.
 *
 * @param {string} packageDir - 패키지 루트
...
async function packSizes(packageDir) {
  ...
  try {
    ...
    if (fileName === undefined) throw new FontBudgetError('pack-failed', `pnpm pack 뒤 ${tmp}에 .tgz 파일이 없습니다.`);
...
/**
 * Elements dist의 측정값을 모두 모읍니다.
 *
 * @param {string} packageDir - 패키지 루트 (`packages/elements`)
 * @param {{ pack?: boolean }} [options] - `pack: false`면 `pnpm pack`을 생략합니다(tarball·unpacked 항목이 빠집니다)
...
/**
 * 측정값을 예산과 비교합니다. 숫자가 없는 항목(pack 생략)은 건너뜁니다.
 *
 ...
 * @returns {{ ok: boolean, rows: Array<{ item: string, actual: number, limit: number, ok: boolean }> }} 항목별 결과. `item`은 표 제목으로 그대로 씁니다.
 * @throws {FontBudgetError} `classification-failure` - 예산에 있는 청크나 폰트 이름이 측정값에 없을 때
 */
export function checkFontBudget(measurements, budget = FONT_BUDGET) {
  ...
  add('루트 정적 의존 파일 원본', measurements.rootClosure?.raw, budget.rootClosure?.raw);
  add('루트 정적 의존 파일 gzip', measurements.rootClosure?.gzip, budget.rootClosure?.gzip);
  for (const [key, limits] of Object.entries(budget.chunks ?? {})) {
    const chunk = measurements.chunks?.[key];
    if (chunk === undefined) throw new FontBudgetError('classification-failure', `예산 항목 ${key} 청크가 측정값에 없습니다.`);
  ...
  for (const [name, limit] of Object.entries(budget.decoded ?? {})) {
    ...
    if (actual === undefined) throw new FontBudgetError('classification-failure', `예산에 있는 폰트 ${name}이 측정값에 없습니다.`);
