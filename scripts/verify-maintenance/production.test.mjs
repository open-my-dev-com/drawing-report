...
const FINDINGS = [
  { kind: 'files', file: 'scripts/left.mjs', name: 'scripts/left.mjs' },
...
describe('renderProductionReport', () => {
  it('지적이 없으면 허용 목록 건수를 적는다', () => {
    const text = renderProductionReport({ unexpected: [], stale: [] }, formatFinding);
    assert.match(text, new RegExp(`허용 목록 ${PRODUCTION_EXPORT_ALLOWLIST.length}건`));
  });

  it('지적이 있으면 종류별로 나눠 적는다', () => {
    const text = renderProductionReport({ unexpected: exportFindings(FINDINGS), stale: ['낡음'] }, formatFinding);
